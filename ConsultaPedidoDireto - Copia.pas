...
uses
  Windows, SysUtils,  Mask, ToolEdit, Grids, DBGrids, SUIButton,
  ...
  sPanel, Messages, Variants,  SUIDBCtrls, XDBGrids, FDvPanel, CurrEdit, frxClass, frxDBSet,
...
uses ModulodeDados, Z_RotinasGerais, ModulodeDadosConsultas3, Principal,
  ...
  gerNFce, DMOS1, DUPLICATA80COL, DocsFiscais, ConsCfop, extenso1;
          ...
          //   DM.Conexao.Commit;
            { if Sds_pedidos.RecordCount = 0 then
             ...
             Sds_pedidos.CancelRange;
                      ...
                      formVendas.Sds_pedidos_itens.Next;
                      formVendas.Edit60.Text:= FloatToStr(A);
                      formVendas.Edit62.Text:= FloatToStr(A - formVendas.DESC );
...
begin
ShowMessage('O usuário conectado não possui autorização para Excluir o Registro. Contate o Administrador do Sistema para obter acesso!!! !!!');
Exit;
End;
...
e, tam:Integer;
 ...
 Xt,  PercentualDesconto, TOTAL1, TOTAL_VENDA: real;
begin
   DM.sds_vendas.DisableControls;
...
begin
 TRY
  DM.SDS_CFOP.Active := False;
  DM.SDS_CFOP.DataSet.CommandText:='select * from CFOP where codigo_cfop > 4999';
  DM.SDS_CFOP.Active := true;
...
begin
dm.Sds_vendas.Edit;
if DM.SDS_CFOP.Locate('CODIGO_CFOP', RzDBButtonEdit1.Text,[])=True then
 begin
   sedit1.Text := DM.SDS_CFOPDESCRICAO.Text;
 end else
 if not DM.SDS_CFOP.Locate('CODIGO_CFOP', RzDBButtonEdit1.Text,[])=True then
 begin
...
procedure TFormConsultaPedidoDireto.suiDBGrid1CellClick(Column: TColumn);
begin
// VENDA := venda;
//DM.sds_vendas.Edit;
{if DM.sds_vendasCHEK_BOX.AsString = 'T' then
