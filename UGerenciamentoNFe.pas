...
uses
  ...
  System.ImageList, AdvGlowButton, Data.DBXFirebird;
...
uses  ModulodeDados, ModuleDados1, USendEmail, VMXCCePrint, Principal,
...
begin
  ...
  try
    try
      ...
      begin
        ...
        dm.ACBrNFe1.DANFE.Logo := trim(fieldbyname('NFE_LOGOMARCA').AsString);
  ...
  try
    ...
    dm.QConsulta.sql.Text := 'SELECT * FROM NOTA_FISCAL_CCE WHERE COD=:COD and COD_EMPRESA =:COD_EMP';
  ...
  begin
    ...
    begin
      ...
      dm.QConsulta.sql.Text :=
        ...
        'HORA_AUT=:HORA, RETORNO=:RET, STATUS_TRANSMITIDA=:S where COD=:COD and cod_empresa =:cod_emp';
      dm.QConsulta.Parambyname('PROT').AsString := Protocolo;
      dm.QConsulta.Parambyname('DATA').AsString := FormatDatetime('mm/dd/yyyy', DataHora);
      dm.QConsulta.Parambyname('HORA').AsString := FormatDatetime('hh:mm:ss', DataHora);
    ...
    begin
      ...
      try
        ...
        dm.QConsulta.sql.Clear;
        dm.QConsulta.sql.Text :=
          'update nota_fiscal_cce set STATUS_TRANSMITIDA=:S where not COD=:COD and COD_NF=:COD_NF and cod_empresa =:cod_emp';
        dm.QConsulta.Parambyname('COD').AsInteger := COD;
...
{  DM.ACBrNFeDANFEFR1.FastFile := '\rel\DANFeRetrato.fr3';
...
var
  ...
  vEmailFrom,
  vEmailAssunto,
  vEmailTo,  ano, mes, Dia, ie: String;
  vEmailMensagem,
  vEmailCc: Tstrings;
...
//  vCaminhoXML:= DM.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' +
 //   sCNPJEmpresa + '\NFe\' + vDt + '\' + Trim(QConsNfeCHAVE_ACESSO_NFE.AsString) + '-nfe.xml';
     vCaminhoXML:= DM.SDS_CONFIGURACOESPASTA_TRANSMITIDAS_DANFE.Text + '\' +
        sCNPJEmpresa +'\'+ ie + '\NFe\'+ ano +'\' + mes +'\' + Dia + '\' + trim(QConsNfeCHAVE_ACESSO_NFE.AsString) + '-nfe.xml';
        ...
        if dm.ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.Items[0]
         ...
         with ibsql do
          begin
           close;
            commandtext.Clear;
            commandtext.Add('update NFE_NOTA set NFE_NOTA.nr_pedido=''-1'' where  nr_nota = :nfe');
            Parambyname('nfe').Asinteger:= iCodigoNfe;
            open;
            end;
            dm.IBTransaction.CommitRetaining;
    ...
    if dm.IBTransaction.Active then
     ...
     dm.IBTransaction.StartTransaction;
      ...
      begin
         close;
         commandtext.Clear;
         commandtext.Text:= 'UPDATE NFE_NOTA SET STATUS =:STATUS, FL_IMPRESSO =:IMP, DEPEC =:DEPEC, NR_PROTOCOLO_NFE =:PROT, CHAVE_ACESSO_NFE =:CHAVE, CHAVE2 =:CHAVE2, MODELO =:MOD WHERE NR_NOTA = :COD_NF';
...
var
  ...
  tempdata, Dia, diac, ano, anoc, mes, mesc, ie: string;
  CCeImp1: TCCeImp;
  PastaPDFs: String;
begin
 ...
 CCeImp1.Free;
      ...
      anoc := formatdatetime('yyyy', QCartasDATA_AUT.AsDateTime);

      mesc:= formatdatetime('mm', QCartasDATA_AUT.AsDateTime);
      Diac := formatdatetime('dd', QCartasDATA_AUT.AsDateTime);
    { verifica se o arquivo xml da nota existe no caminho especificado }
     sCNPJEmpresa := RemoveChar(DM.SDS_EmpresaCNPJ.Text);
     ie := RemoveChar(DM.SDS_EmpresaIE.Text);
 ...
 //   formatdatetime('yyyymm', QCartasDATA.AsDateTime)+ '\CCe\' + trim(QConsNfeCHAVE_ACESSO_NFE.AsString) + '110110' +
     sCaminhoXMLEvento := DM.SDS_CONFIGURACOESPASTA_TRANSMITIDAS_DANFE.Text+ '\' +
    sCNPJEmpresa +'\' + '\NFe\'+ anoc +'\' + mesc +'\' + Diac + '\' + '110110' + trim(QCartaschave.AsString) +
    RetZero(QCartasSEQ.AsString,2) + '-ProcEventoNFe.xml';
  ...
  begin
    ShowMessage('Arquivo XML do Evento não encontrado! voce podera selecionar agora..');
  OpenDialog2.Title := 'Selecione o Evento';
  OpenDialog2.DefaultExt := '*.XML';
  OpenDialog2.Filter := 'Arquivos XML (*.XML)|*.XML|Todos os Arquivos (*.*)|*.*';
  OpenDialog2.InitialDir := DM.SDS_CONFIGURACOESPASTA_TRANSMITIDAS_DANFE.Text;
 if OpenDialog2.Execute then
 begin
     sCaminhoXMLEvento  := OpenDialog2.FileName;
 end;
   // exit;
  end;
...
var
sCaminhoArquivo , ano, mes, dia, ie: string;
begin
{DM.ACBrNFeDANFEFR1.FastFile := 'C:\Siace\rel\DANFeRetrato.fr3';
DM.ACBrNFeDANFEFR1.FastFileEvento := 'C:\Siace\rel\EVENTOS.fr3'; }
//DM.ACBrNFeDANFEFR1.tipoDanfe := tiRetrato;
...
begin
  ...
  try
    ...
    begin
      ...
      ICartas.sql.Clear;
      ICartas.sql.Text :=
        'select MAX( SEQ ) from NOTA_FISCAL_CCE where COD_NF=:COD_NF and cod_empresa =:cod_emp';
      ICartas.Parambyname('COD_NF').AsInteger := QConsNfeNR_NOTA.AsInteger;
      icartas.ParamByName('COD_EMP').Asinteger := Iemp;
      ICartas.Open;
    ...
    begin
      ...
      begin
        ...
        ICartas.sql.Text := 'Insert into NOTA_FISCAL_CCE(COD_NF,SEQ,CHAVE,' +
          'STATUS_TRANSMITIDA, DATA, CORRECAO, COD_EMPRESA) values( :COD_NF,:SEQ,:CHAVE,' +
          ':STATUS_TRANSMITIDA, :DATA, :CORRECAO, :cod_emp)';
        ...
        qcartas.ParamByName('COD_EMP').Asinteger := Iemp;
        QCartas.Open;
      end;
    end;
  finally
...
procedure TFrmGerenciamentoNFe.DBGrid2DrawColumnCell(Sender: TObject;
  ...
  State: TGridDrawState);
begin
 if Column.field.FieldName = 'STATUS_TRANSMITIDA' then // o nome do campo
  begin
