...
uses
  ...
  rxToolEdit, rxCurrEdit, RXDBCtrl, FMTBcd, SqlExpr, FireDAC.Stan.Intf,
  FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS,
  FireDAC.Phys.Intf, FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt,
  FireDAC.Comp.DataSet, FireDAC.Comp.Client, AdvGlowButton;
...
type
  TFrmDadosAdm = class(TForm)
    ...
    BtnConsBandeira: TAdvGlowButton;
    ...
    QBuscaParcelasHISTORICO: TStringField;
    FGravaParcela: TFDCommand;
...
Uses ModulodeDados, Ubibli1, UConsAdmCartaoCredito, ConsClientes,
  UAlteraParcelaCTR, Principal, UConsBandeiraCartao;
...
IF EdtDoc.Text >'' then
   ...
   QBuscaParcelas.SQL.Clear;
   QBuscaParcelas.SQL.Add('SELECT C.CODIGO, C.EMISSAO, C.VENCIMENTO, C.VALOR, C.TAXA, B.DESCRICAO, C.HISTORICO FROM CONTAS_RECEBER_ADM_CARTAO C'+'');
   QBuscaParcelas.SQL.Add('INNER JOIN BANDEIRAS_CARTAO B ON (C.CODIGO_BANDEIRA = B.CODIGO) WHERE C.DOCUMENTO = :CODVEN ORDER BY C.CODIGO ');
   with QBuscaParcelas do
      begin
         close;
         Parambyname('codven').AsString:= EdtDoc.Text;
         Open;
      end;
end;
end;
...
begin
      ...
      begin
       ...
       dm.ProxCod.SQL.Add('select max(codigo) as N_CODIGO FROM CONTAS_RECEBER_ADM_CARTAO');
       DM.ProxCod.Open;
      ...
      begin
         ...
         try
           try
             with qGravaParcela do
                begin
                   ...
                   Parambyname('HISTORICO').AsString   := 'Recebimento no Cartão Ref: a Venda nº '+ EdtVenda.Text;
           ...
           QGravaParcela.Close;
                    ...
                    qrcaixa_mov.parambyname('data').asdatetime      :=  StrToDate(dt_venda.text);
                    ...
                    if Self.Tag = 12 then
                    begin
                      qrcaixa_mov.parambyname('tipo_mov').asINTEGER:= 12;
                    end
                    else
                    begin
      ...
      begin
         ...
         try
           try
             with QGravaParcela do
                begin
                   ...
                   parambyname('DOC').Value            := DM.ProxCodN_CODIGO.ASINTEGER + 1;
                   ExecSQL;
                end;
             DM.IBTransaction.Commit;
          except
           ...
           QGravaParcela.Close;
                    ...
                    qrcaixa_mov.parambyname('historico').asstring := 'Rec. no Crediario No. '+EdtDoc.Text+' - '+EdtCli1.Text;
                    qrcaixa_mov.parambyname('tipo').AsString  := 'F';
                    qrcaixa_mov.parambyname('origem').AsString  := 'FIN';
                      if EdtVenda.Text > '' then
                     begin
      ...
      begin
         ...
         try
           try
             with QGravaParcela do
                begin
                   ...
                   parambyname('taxa').AsCurrency      := cTaxa;
                   Parambyname('codcli').AsInteger     := strtoint(EdtCli.Text);
                   Parambyname('documento').Value      := EdtDoc.Text;
                   Parambyname('parcela').AsInteger    := i;
                   Parambyname('usuario').AsString     := Formprincipal.userlogado;
                   Parambyname('HISTORICO').AsString   := Memo1.Text;
                   parambyname('SEQ').Value            := NULL;
           ...
           QGravaParcela.Close;
                    ...
                    qrcaixa_mov.parambyname('historico').asstring := 'Rec. no Cartão No. '+EdtDoc.Text+' - '+EdtCli1.Text;
                    qrcaixa_mov.parambyname('tipo').AsString  := 'F';
                    qrcaixa_mov.parambyname('origem').AsString  := 'FIN';
                      if EdtVenda.Text > '' then
                     begin
...
begin
 if Application.MESSAGEBOX('Parcelas não geradas favor gerara as parcelas deseja realmente sair?', 'Confirmar', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON1) = ID_YES then
begin
  cadastrouch := false;
end;
end else
