...
uses
  ...
  Dialogs,  ExtCtrls, StdCtrls,  Menus,  ImgList,  DB, Data.DBXFirebird, SqlExpr,
  ...
  IBCustomDataSet, IBQuery, FireDAC.Stan.Intf, FireDAC.Stan.Option, DBClient,
  ...
  TFormProdutos = class(TForm)
    ...
    RxLabel112: TRxLabel;
    QRRENTABILIDADERENDIMENTO: TFMTBCDField;
    QRRENTABILIDADEVALOR: TFMTBCDField;
    QRRENTABILIDADEQUANTIDADE: TFMTBCDField;
    qrProdutos_ImovelVALOR_ALUGUEL: TFMTBCDField;
    qrProdutos_ImovelVALOR_VENDA: TFMTBCDField;
    qrProdutos_ImovelPERC_IMPOSTO: TFMTBCDField;
    RxLabel113: TRxLabel;
    TabSheet9: TRzTabSheet;
    DBCheckBox9: TDBCheckBox;
    db_mestre: TRxDBComboEdit;
    edt_mestre: TEdit;
    ...
    procedure db_mestreExit(Sender: TObject);
    procedure db_mestreButtonClick(Sender: TObject);
...
uses ModulodeDados, Z_RotinasGerais, Principal, Acesso, ConsGrupos,
  ...
  CONSULTACEST, ConsProdutosVendas;
...
procedure TFormProdutos.db_mestreButtonClick(Sender: TObject);
begin
...
// FormConsProdutosVendas.Edit1.Text := ComboEdit1.Text;
 ...
 begin
 dm.SDS_PRODUTOSCOD_MESTRE.TEXT := DM.SDS_PRODUTOS_cCODIGO.Text;
 edt_mestre.Text := DM.SDS_PRODUTOS_cdescricao.Text;
end;
end;

procedure TFormProdutos.db_mestreExit(Sender: TObject);
begin
 if db_mestre.Text >'' THEN
BEGIN
// EDIT1.Text:= lEstoque.Text;
edt_mestre.Text:= consulta2('produtos', dbt_codigo, 'codigo', 'descricao', dm.IBTransaction, dm.qConsulta);
end;
end;
...
begin
  ...
  DMC.SDS_PDV.sql.Clear;
  DMC.SDS_PDV.sql.add('select * from produtos where  DESCRICAO like ' + #39 + '%' + db_nome.Text + '%' + #39+'ORDER BY codigo');
  DMC.SDS_PDV.Active := True;

if DMC.SDS_PDV.Locate('DESCRICAO',db_nome.Text, []) = true then
BEGIN
 MessageDlg('Existe um produto cadastrado com esta descricao Favor informar outra descricao', mtInformation,[mbOk], 0);
 DB_NOME.SetFocus;
end;
end;
end;
...
begin
 ...
 //  cod :=Prxcod('produtos', 'codigo', dm.IBTransaction, dm.ibsqlcod);
   ...
   DM.ProxCod.Open;
   cod:=   dm.ProxCodN_CODIGO.AsInteger;


   DM.SDS_Produtos.Insert;
   DM.SDS_ProdutosCODIGO.AsInteger := Cod + 1;//DM.ProxCodN_CODIGO.ASINTEGER + 1;
   DM.SDS_ProdutosPROCURA.AsInteger := Cod + 1;
   DM.SDS_PRODUTOSDESCRICAO.Text := '';
   DM.SDS_PRODUTOSCODIGO_BARRAS.text:= Formatar(dbt_codigo.Text,5,False,'0');
   DM.sds_PRODUTOSCOD_EMPRESA.AsInteger:= iemp;
   ...
   DM.SDS_ProdutosPRECO_CUSTO.Text:= '0,001';
...
begin
           ...
           //   dbt_codigo.Text := inttostr(prxcod('produto', 'cod_pro', dm.IBTransaction, dm.ibsqlcod));
             { dm.ProxCod.Close;
              dm.ProxCod.SQL.Clear;
              dm.ProxCod.SQL.Add('select max(codigo) as N_CODIGO FROM PRODUTOS');
              DM.ProxCod.Open;  }
              ...
              DM.SDS_PRODUTOS.edit;
              DM.SDS_ProdutosCODIGO.ASINTEGER := cod;//DM.ProxCodN_CODIGO.ASINTEGER + 1;
              DM.SDS_PRODUTOSDATAULTALTERACAO.AsDateTime     := Date;
              DM.SDS_PRODUTOSDATACAD.AsDateTime              := Date;
              DM.SDS_PRODUTOSUSERCAD.AsString                := FormPrincipal.UserLogado;
              DM.SDS_PRODUTOSCOD_EMPRESA.AsInteger           := iemp;
...
{if DM.IBTransaction.Active then
  ...
  // try
        ...
        begin
           ...
           if bGrava then
              ...
              //   cod_pro.Text:= inttostr(prxcod('produto', 'cod_pro', dm.IBTransaction, dm.ibsqlcod));
                 ...
                 CommandText.text := ('INSERT INTO PRODUTOS(CODIGO, CODIGO_BARRAS, DESCRICAO, ABREVIADO, PRECO_CUSTO, MARGEM_LUCRO,'+
              ...
              begin
                 CommandText.Text := ('UPDATE PRODUTOS SET CODIGO_BARRAS = :CODIGO_BARRAS, DESCRICAO = :DESCRICAO, ABREVIADO = :ABREVIADO,  PRECO_CUSTO = :PRECO_CUSTO,'+
    ...
    DM.fconsulta.Close;
              ...
              {   dmc.SDS_PDV.Active := False;
                 dmc.SDS_PDV.Active := true;
                 if DMC.SDS_PDV.Locate('CODIGO',dbt_codigo.Text, []) = false then
                 BEGIN
                 MessageDlg(' Nao Existe um produto cadastrado com este Código Favor informar outro Código EAN', mtInformation,[mbOk], 0);
                  Exit;
                  Abort;
                 end;      }
...
begin
  DMC.SDS_PDV.Active := false;
  DMC.SDS_PDV.sql.Clear;
  DMC.SDS_PDV.sql.add('select * from produtos where  CODIGO_BARRAS like ' + #39 + '%' + DBEdit1.Text + '%' + #39+'ORDER BY codigo');
  DMC.SDS_PDV.Active := True;
...
//  lEstoque2.Text:= consulta('motivo_acerto', edit1, 'cod_mot', 'nome_mot', dm.IBTransaction, dm.qConsulta);
  ...
  if lEstoque2.Text = '' then
     begin
        showmessage('Local de estoque não localizado');
        lEstoque.SetFocus;
     end;
end;
end;
...
procedure TFormProdutos.DBEdit25Exit(Sender: TObject);
begin
if DBEdit25.TEXT >'' then
 BEGIN
  if Length(DBEdit25.Text) < 8  then
 begin
   ShowMessage('NCM DEVE CONTER 8 POSICOES ') ;
   DBEdit25.SetFocus;
 end;
end;
end;
...
begin
  DMC.SDS_PDV.Active := false;
  DMC.SDS_PDV.SQL.clear;
  DMC.SDS_PDV.SQL.add('select * from produtos where  REFERENCIA like ' + #39 + '%' + DBEdit2.Text + '%' + #39+'ORDER BY codigo');
  DMC.SDS_PDV.Active := True;
...
begin
  ...
  if Panel5.Enabled = True then SUIDBEdit14.SetFocus;
     ...
     if suiDBEdit14.text >'0' then
      begin
      custo:=  DM.SDS_PRODUTOSPRECO_CUSTO.AsCurrency;
      lucro:= DM.SDS_PRODUTOSpreco_VENDA.Asfloat;
...
begin
 ...
 DBEdit20.SetFocus;
      ...
      begin
      custo:= DM.SDS_PRODUTOSPRECO_CUSTO.AsCurrency;
      lucro:=DM.SDS_PRODUTOSpreco_VENDA2.Ascurrency;
      valor:=((lucro * 100)/custo) - 100;
