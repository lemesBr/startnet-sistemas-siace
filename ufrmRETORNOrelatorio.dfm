inherited frmRETORNOrelatorio: TfrmRETORNOrelatorio
  ...
  object qryDefaultITEM: TFDQuery
    ...
    SQL.Strings = (
      ...
      '  clientes.nome_rs AS RAZAOSOCIAL,'
      '  clientes.cpf_cnpj as cnpj'
      'FROM'
      '  CBR_RETORNO_TITULOS'
      
        '    LEFT JOIN CREDIARIO ON CBR_RETORNO_TITULOS.TITULO = CREDIARI' +
        'O.numboleto1 AND CBR_RETORNO_TITULOS.ID_EMPRESA = CREDIARIO.COD_' +
        'EMPRESA'
      
        '    LEFT JOIN CLIENTES      ON CREDIARIO.cod_empresa     = clien' +
        'tes.COD_empresa          AND CREDIARIO.codigo_cliente = CLIENTES' +
        '.codigo'
      'WHERE'
      '  1=1'
