...
uses
  ...
  Dialogs, ExtCtrls,  SUIButton, StdCtrls, jpeg, RXCtrls,
  SUIDlg, DB, Mask, ACBrBase, Buttons, RxToolEdit, sLabel;
...
Try
 ...
 FormPrincipal.NFCE           :=  DM.SDS_Usuarios.FieldByname('UTILIZA_NFCE').AsString;
 FormPrincipal.bloqueia       :=  DM.SDS_Usuarios.FieldByname('BLOQUEIA_VENDA').AsString;
...
FormPrincipal.FormaoAlteraodePreos1.Enabled :=False;
end else
...
BEGIN
FormPrincipal.s141.Enabled:= TRUE;
end else
begin
FormPrincipal.s141.Enabled:= False;
end;

If DM.SDS_UsuariosS142.Text = 'S' THEN
BEGIN
FormPrincipal.s142.Enabled:= TRUE;
end else
begin
FormPrincipal.s142.Enabled:= False;
end;

If DM.SDS_UsuariosS143.Text = 'S' THEN
BEGIN
FormPrincipal.s143.Enabled:= TRUE;
end else
begin
FormPrincipal.s143.Enabled:= False;
end;
//////////////////////////////////////////////
 ...
 Close;
   ...
   // Informacao.ShowModal;
    ShowMessage('Não foi possivel logar no sistema, verifique o usuario e a senha..');
    edit3.SetFocus;
    end;
except
...
Close;
    ...
    if Application.MESSAGEBOX('Deseja Realmente Fechar o Sistema?', 'Confirmar', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON1) = ID_no then
  begin
    CanClose := False;
  end else
  BEGIN
