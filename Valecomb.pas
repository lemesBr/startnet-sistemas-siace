...
uses
  ...
  RXDBCtrl, Mask, DBCtrls, aDvGlowButton, db, ExtCtrls ,
  Grids, XDBGrids, SUIDlg, RXCtrls, RzEdit, RzBtnEdt,
  FireDAC.Stan.Intf, FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS,
  FireDAC.Phys.Intf, FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet,
  FireDAC.Comp.Client, RzDBEdit, RxDBComb, RxToolEdit, AdvPanel;

type
  TFormValeComb = class(TFrmCadastroNovo)
    Label1: TLabel;
    DBEdit1: TDBEdit;
    Label2: TLabel;
...
Procedure TFormValeComb.AtualizaManutencao;
begin
...
procedure TFormValeComb.HabilitaPanel;
begin
  Panel1.Enabled:= True;
end;

procedure TFormValeComb.DesabilitaPanel;
begin
  Panel1.Enabled:= False;
end;

procedure TFormValeComb.HabilitaNovo;
begin
...
procedure TFormValeComb.RxDBComboEdit1ButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TFormValeComb.RxDBComboEdit1KeyDown(Sender: TObject;
...
procedure TFormValeComb.btnNovoClick(Sender: TObject);
begin
  inherited;
...
procedure TFormValeComb.BtnAlterarClick(Sender: TObject);
begin
  inherited;
...
procedure TFormValeComb.BtnCancelarClick(Sender: TObject);
begin
  inherited;
  try
...
procedure TFormValeComb.BtnExcluirClick(Sender: TObject);
begin
  inherited;
...
procedure TFormValeComb.BtnGravarClick(Sender: TObject);
begin
  inherited;
  try
    DM1.Sds_valeTrocoCOD_VEICULO.AsString := veiculo.Text;
    DM1.Sds_valeTrocoTOTAL_VENDA.AsFloat := 0;
    DM1.Sds_valeTrocoTOTAL_PAGO.AsFloat := 0;
    DM1.Sds_valeTrocoSALDO_ATUAL.AsFloat := EvDBNumEdit1.Value;
...
procedure TFormValeComb.BtnConsultarClick(Sender: TObject);
begin
  dm1.Sds_valeTroco.Open;
...
procedure TFormValeComb.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key =#27 then
    close;
...
procedure TFormValeComb.FormClose(Sender: TObject;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FormRelValeTroco.RLLabel35.Caption := 'Reconheço(emos) a exatidão deste '+
        'VALE, na importancia acima Recebida.';
      end else
      begin
        FormRelValeTroco.RLLabel1.Caption := 'VALE TROCO   Nº';
        FormRelValeTroco.RLLabel35.Caption := 'Reconheço(emos) a exatidão deste '+
        'VALE TROCO, na importancia acima.';
      end;
...
procedure TFormValeComb.FormShow(Sender: TObject);
begin
...
procedure TFormValeComb.RxDBComboEdit1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TFormValeComb.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
  if (Key=VK_F2) then
...
procedure TFormValeComb.EvDBComboBox1Change(Sender: TObject);
begin
  inherited;
...
procedure TFormValeComb.RxDBComboEdit1Exit(Sender: TObject);
begin
  inherited;
  if suiDBRadioGroup2.Enabled = True then
    suiDBRadioGroup2.SetFocus;
end;

procedure TFormValeComb.EvDBComboBox2Exit(Sender: TObject);
begin
  if EvDBComboBox1.ItemIndex = 0 then
  begin
    if (suiDBRadioGroup2.ItemIndex = 2) OR (suiDBRadioGroup2.ItemIndex = 3) then
    begin
  ...
  if EvDBComboBox1.ItemIndex = 1 then
  begin
    if (suiDBRadioGroup2.ItemIndex = 0) OR (suiDBRadioGroup2.ItemIndex = 1) then
    begin
...
procedure TFormValeComb.BtnAddProClick(Sender: TObject);
begin
  FormClientes:=nil ;
...
procedure TFormValeComb.SpeedButton1Click(Sender: TObject);
begin
...
procedure TFormValeComb.veiculoButtonClick(Sender: TObject);
begin
  inherited;
  DM.SDS_VEICULOS.Active:= False;
  dm.SDS_veiculos.SQL.Clear;
...
procedure TFormValeComb.veiculoChange(Sender: TObject);
begin
  inherited;
...
procedure TFormValeComb.veiculoExit(Sender: TObject);
begin
  inherited;
  if  veiculo.Text >'' then
  begin
...
procedure TFormValeComb.suiDBRadioGroup2Enter(Sender: TObject);
begin
  inherited;
  suiDBRadioGroup2.ItemIndex := 0;
end;
