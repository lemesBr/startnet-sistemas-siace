...
type
  ...
  private
    ...
    bar: array[0..5] of   TRLBarcode;
    procedure AddIfNotBlank(memo: TRLMemo; s: string);
    procedure AddIfNotBlank1(bar: TRLBarcode; s: string);
  public
    { Public declarations }
      LabelSize : THLabel;
      SkipRow: integer;
...
var
  FormEtiqBarras: TFormEtiqBarras;
...
procedure TFormEtiqBarras.RLBand1BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
var
  nIdx : integer;
begin
   if LabelSize = h5165 then begin
...
procedure TFormEtiqBarras.FormCreate(Sender: TObject);
begin
  {
...
procedure TFormEtiqBarras.RLReport2PageStarting(Sender: TObject);
begin
  {
    We have an array of TQRMemo and we assign each element to the
    existing memos on the report.  This allows us to set the memo
...
procedure TFormEtiqBarras.RLReport2BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
var
  nIdx: integer;
begin
...
procedure TFormEtiqBarras.AddIfNotBlank(memo: TRlMemo; s: string);
begin
  { simple function to only add non blank fields }
  if Trim(s) <> '' then
    memo.Lines.Add(s);
...
procedure TFormEtiqBarras.AddIfNotBlank1(bar: TRLBarcode; s: string);
begin
  { simple function to only add non blank fields }
  if Trim(s) <> '' then
    bar.Caption := FormEtqProdutos.ClientDataSet2Codigo_barras.AsString;
end;
end.
