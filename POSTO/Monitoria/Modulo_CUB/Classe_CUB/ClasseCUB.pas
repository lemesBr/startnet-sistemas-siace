...
uses ExtCtrls, CUBUtils, SysUtils, UnitFuncoes, MMSystem, ClasseBomba, ClasseBico,
  ...
  public
   ...
   Procedure Cria_Bico(id_Bomba, id_Bico, id_Produto, cod_produto, cod_Tanque: Byte; Nome_Produto: String; Unidade: ShortString);
...
//--- Cria Objeto Bico
Procedure TCUB.Cria_Bico(id_Bomba, id_Bico, id_Produto, cod_produto, cod_Tanque: Byte; Nome_Produto: String; Unidade: ShortString);
begin
 id_Bomba := id_Bomba - 1;
 id_Bico  := id_Bico - 1;
 ...
 FBomba[id_Bomba].Bico[id_Bico].Unidade      := Unidade;

 //Lê encerrantes das bombas ao iniciar (somente bombas no estado autorizada)
 if FBomba[id_Bomba].StatusBomba in [SAUTORIZADA] then
  FBomba[id_Bomba].Bico[id_Bico].LerTotais(False);
end;
