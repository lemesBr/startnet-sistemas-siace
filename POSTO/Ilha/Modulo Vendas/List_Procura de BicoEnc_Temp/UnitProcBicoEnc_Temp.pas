...
uses UnitDataM1, Unit13Venda, ClasseCUB, UnitFrmPrincipal, UnitFuncoes,
  CUBUtils, ClasseBomba;//, Unit13Venda;
...
begin
 DM1.CloseIBArray([VendaBicoEnc_TempListFrm.QryBicoEnc_Temp, VendaBicoEnc_TempListFrm.QryBomba]);
 action := cafree;
end;
...
begin
 ...
 for x := 0 to VendaFrm.Listview1.items.Count - 1 do
 begin
  if VendaFrm.Listview1.Items.Item[x].SubItems.Strings[25] <> '' then
  begin
    ...
    QryBicoEnc_Temp.ParamByName('COD_BICOENC_TEMP' + inttostr(x)).AsString :=
               VendaFrm.Listview1.Items.Item[x].SubItems.Strings[25];
  end;
 end;
