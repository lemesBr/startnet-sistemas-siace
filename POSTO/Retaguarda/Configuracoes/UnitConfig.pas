...
uses UnitDataM1, UnitFuncoes, UnitFrmPrincipal;
...
try
 ...
 //------ Config Logo ------
 Edit19.Text := ArquivoIni.ReadString('IMAGEM', 'LogoPath', extractfiledir(application.Exename) + 'Logo.JPG');
 image1.Picture.LoadFromFile(ArquivoIni.ReadString('IMAGEM', 'LogoPath', extractfiledir(application.Exename) + 'Logo.JPG'));
...
//   ArquivoIni.WriteString('BOLETO', 'Cabecalho4', edit25.Text);
   ...
   //------ Config Imagem ------
   if ArquivoINI.ReadString('IMAGEM', 'LogoPath', Edit19.Text) <> Edit19.Text  then
    ArquivoIni.WriteString('IMAGEM', 'LogoPath', Edit19.Text);
...
if sender = bitbtn1 then //Caminho da Base de Dados
 begin
  ...
  dm1.opendialog1.Filter := 'Banco de Dados (*.ib)|*.ib';
...
{ if sender = bitbtn10 then //Caminho da Base de Dados Remota
 ...
 begin
  ...
  dm1.opendialog1.InitialDir := ExtractFileDir(Application.ExeName);
  dm1.opendialog1.Filter := 'Base Local (*.ib)|*.ib';
  if dm1.opendialog1.Execute then
...
if sender = bitbtn2 then // Caminho da imagem
 begin
  dm1.opendialog1.Title := 'Logomarca';
  dm1.opendialog1.InitialDir := ExtractFileDir(Application.ExeName);
  dm1.opendialog1.Filter := 'Imagem (*.JPG)|*.JPG';
  if dm1.opendialog1.Execute then
   begin
    edit52.Text := dm1.opendialog1.FileName;
    image1.Picture.LoadFromFile(dm1.opendialog1.FileName);
   end;
 end;
end;
