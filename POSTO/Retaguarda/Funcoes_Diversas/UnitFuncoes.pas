...
begin
 try
   ...
   logfilename := extractfilepath(application.ExeName) + 'TKSLog' +
   ...
   if not fileexists(logfilename) then
    begin
     rewrite(f);
     writeln(f, '.LOG');
     writeln(f, 'TKS Software');
     writeln(f, 'TeleFax: +55(71)371-1818');
     writeln(f, 'E-Mail : tks@e-net.com.br');
     writeln(f, 'Site   : www.igara.com.br');
...
begin
 try
   ...
   //------ Config Produto ------
   if not ArquivoIni.ValueExists('PRODUTO', 'und_default') then
    ArquivoIni.WriteString('PRODUTO', 'und_default', 'Un');

   //------ Config Nº Serie ------
   if not ArquivoIni.ValueExists('NSerie', 'EnableNSerie') then
     ArquivoIni.WriteBool('NSerie', 'EnableNSerie', False);
   if not ArquivoIni.ValueExists('NSerie', 'EnableNSerieRef') then
     ArquivoIni.WriteBool('NSerie', 'EnableNSerieRef', False);
   ...
   //------ Config Discador ------
   if not ArquivoIni.ValueExists('Discador', 'ModemStartString1') then
     ArquivoIni.WriteString('Discador', 'ModemStartString1', ' ');
   if not ArquivoIni.ValueExists('Discador', 'ModemStartString2') then
     ArquivoIni.WriteString('Discador', 'ModemStartString2', ' ');
   if not ArquivoIni.ValueExists('Discador', 'ModemStartString3') then
     ArquivoIni.WriteString('Discador', 'ModemStartString3', ' ');
   if not ArquivoIni.ValueExists('Discador', 'COM') then
     ArquivoIni.WriteInteger('Discador', 'COM', - 1);
   if not ArquivoIni.ValueExists('Discador', 'EnableLinhaExterna') then
     ArquivoIni.WriteBool('Discador', 'EnableLinhaExterna', False);
   if not ArquivoIni.ValueExists('Discador', 'LinhaExterna') then
     ArquivoIni.WriteString('Discador', 'LinhaExterna', ' ');
   if not ArquivoIni.ValueExists('Discador', 'PwdLinhaExterna') then
     ArquivoIni.WriteString('Discador', 'PwdLinhaExterna', ' ');
   if not ArquivoIni.ValueExists('Discador', 'DiscagemTom') then
     ArquivoIni.WriteBool('Discador', 'DiscagemTom', True);
   if not ArquivoIni.ValueExists('Discador', 'OperadoraPadrao') then
     ArquivoIni.WriteString('Discador', 'OperadoraPadrao', ' ');
   if not ArquivoIni.ValueExists('Discador', 'EnableOperadoraFixa') then
     ArquivoIni.WriteBool('Discador', 'EnableOperadoraFixa', True);
   if not ArquivoIni.ValueExists('Discador', 'PathTabelaPrecos') then
     ArquivoIni.WriteString('Discador', 'PathTabelaPrecos', ' ');
   if not ArquivoIni.ValueExists('Discador', 'ModemNaoInstalado') then
     ArquivoIni.WriteBool('Discador', 'ModemNaoInstalado', False);
   if not ArquivoIni.ValueExists('Discador', 'CallSolicitante') then
     ArquivoIni.WriteBool('Discador', 'CallSolicitante', False);

   //------ Config Ordem de Serviço ------
   if not ArquivoIni.ValueExists('OS', 'Previsao') then
     ArquivoIni.WriteInteger('OS', 'Previsao', 0);
   if not ArquivoIni.ValueExists('OS', 'Cabecalho1') then
     ArquivoIni.WriteString('OS', 'Cabecalho1', 'TKS Telefonia e Software');
   if not ArquivoIni.ValueExists('OS', 'Cabecalho2') then
     ArquivoIni.WriteString('OS', 'Cabecalho2',
       'Rua B 236 Loja D - Centro Comercial do Imbui Cep: 41.720-050 - Salvador-BA-Brasil');
   if not ArquivoIni.ValueExists('OS', 'Cabecalho3') then
     ArquivoIni.WriteString('OS', 'Cabecalho3',
       'Telefax: (71) 371-1818   E-mail: tks@e-net.com.br   Site: www.igara.com.br');
   if not ArquivoIni.ValueExists('OS', 'Cabecalho4') then
     ArquivoIni.WriteString('OS', 'Cabecalho4', 'ORDEM DE SERVIÇO');
   if not ArquivoIni.ValueExists('OS', 'ImpRodape1') then
     ArquivoIni.WriteString('OS', 'ImpRodape1', '');
   if not ArquivoIni.ValueExists('OS', 'ImpRodape2') then
     ArquivoIni.WriteString('OS', 'ImpRodape2', '');
   if not ArquivoIni.ValueExists('OS', 'ImpRodape3') then
     ArquivoIni.WriteString('OS', 'ImpRodape3', '');
   if not ArquivoIni.ValueExists('OS', 'ImpRodape4') then
     ArquivoIni.WriteString('OS', 'ImpRodape4', '');
   if not ArquivoIni.ValueExists('OS', 'AttRodape1') then
     ArquivoIni.WriteString('OS', 'AttRodape1', '');
   if not ArquivoIni.ValueExists('OS', 'AttRodape2') then
     ArquivoIni.WriteString('OS', 'AttRodape2', '');
   if not ArquivoIni.ValueExists('OS', 'AttRodape3') then
     ArquivoIni.WriteString('OS', 'AttRodape3', '');
   if not ArquivoIni.ValueExists('OS', 'AttRodape4') then
     ArquivoIni.WriteString('OS', 'AttRodape4', '');
   if not ArquivoIni.ValueExists('OS', 'AttRodape5') then
     ArquivoIni.WriteString('OS', 'AttRodape5', '');
   if not ArquivoIni.ValueExists('OS', 'AttRodape6') then
     ArquivoIni.WriteString('OS', 'AttRodape6', '');
   if not ArquivoIni.ValueExists('OS', 'AttRodape7') then
     ArquivoIni.WriteString('OS', 'AttRodape7', '');
   if not ArquivoIni.ValueExists('OS', 'AttRodape8') then
     ArquivoIni.WriteString('OS', 'AttRodape8', '');
   ...
   if not ArquivoIni.ValueExists('ECF', 'MsgP') then
     ArquivoIni.WriteString('ECF', 'MsgP', 'TKS Software www.igara.com.br');
   ...
   //------ Config Nota Fiscal ------
   if not ArquivoIni.ValueExists('NOTAFISCAL', 'NumNota') then
     ArquivoIni.WriteInteger('NOTAFISCAL', 'NumNota', 1);
   if not ArquivoIni.ValueExists('NOTAFISCAL', 'EnablePreview') then
     ArquivoIni.WriteBool('NOTAFISCAL', 'EnablePreview', True);

   //------ Config Orcamento ------
   if not ArquivoIni.ValueExists('ORCAMENTO', 'PortaImpr') then
     ArquivoIni.WriteString('ORCAMENTO', 'PortaImpr', 'LPT1');
   if not ArquivoIni.ValueExists('ORCAMENTO', 'NumVias') then
     ArquivoIni.WriteInteger('ORCAMENTO', 'NumVias', 1);
   if not ArquivoIni.ValueExists('ORCAMENTO', 'EnableCabecalho') then
     ArquivoIni.WriteBool('ORCAMENTO', 'EnableCabecalho', True);
   if not ArquivoIni.ValueExists('ORCAMENTO', 'EnableRodape') then
     ArquivoIni.WriteBool('ORCAMENTO', 'EnableRodape', True);
   if not ArquivoIni.ValueExists('ORCAMENTO', 'EnablePreview') then
     ArquivoIni.WriteBool('ORCAMENTO', 'EnablePreview', True);
   if not ArquivoIni.ValueExists('ORCAMENTO', 'EnableMsgP') then
     ArquivoIni.WriteBool('ORCAMENTO', 'EnableMsgP', True);
   if not ArquivoIni.ValueExists('ORCAMENTO', 'AskImpr') then
     ArquivoIni.WriteBool('ORCAMENTO', 'AskImpr', True);
   if not ArquivoIni.ValueExists('ORCAMENTO', 'TipoImpr') then
     ArquivoIni.WriteInteger('ORCAMENTO', 'TipoImpr', 0);
   if not ArquivoIni.ValueExists('ORCAMENTO', 'DefaultAnswerAskImpr') then
     ArquivoIni.WriteBool('ORCAMENTO', 'DefaultAnswerAskImpr', False);
   if not ArquivoIni.ValueExists('ORCAMENTO', 'Cabecalho') then
     ArquivoIni.WriteString('ORCAMENTO', 'Cabecalho', 'Orcamento');
   if not ArquivoIni.ValueExists('ORCAMENTO', 'MsgP') then
     ArquivoIni.WriteString('ORCAMENTO', 'MsgP', 'TKS Software www.igara.com.br');
   if not ArquivoIni.ValueExists('ORCAMENTO', 'Rodape1') then
     ArquivoIni.WriteString('ORCAMENTO', 'Rodape1', 'TKS Telefonia e Software');
   if not ArquivoIni.ValueExists('ORCAMENTO', 'Rodape2') then
     ArquivoIni.WriteString('ORCAMENTO', 'Rodape2',
       'Rua B 236 Loja D - Centro Comercial do Imbui');
   if not ArquivoIni.ValueExists('ORCAMENTO', 'Rodape3') then
     ArquivoIni.WriteString('ORCAMENTO', 'Rodape3',
       'Cep: 41.720-050   -   Salvador-Bahia-Brasil');
   if not ArquivoIni.ValueExists('ORCAMENTO', 'Rodape4') then
     ArquivoIni.WriteString('ORCAMENTO', 'Rodape4',
       'Telefax: (71) 371-1818   E-mail: tks@e-net.com.br   Site: www.igara.com.br');
   ...
   if not ArquivoIni.ValueExists('BOLETO', 'Cabecalho1') then
     ArquivoIni.WriteString('BOLETO', 'Cabecalho1', 'TKS Software');
   ...
   if not ArquivoIni.ValueExists('BOLETO', 'Cabecalho4') then
     ArquivoIni.WriteString('BOLETO', 'Cabecalho4',
       'www.igara.com.br  /  tksltda@uol.com.br');
   if not ArquivoIni.ValueExists('BOLETO', 'MsgP') then
     ArquivoIni.WriteString('BOLETO', 'MsgP', 'TKS Software www.igara.com.br');
   if not ArquivoIni.ValueExists('BOLETO', 'Rodape1') then
     ArquivoIni.WriteString('BOLETO', 'Rodape1', 'TKS Telefonia e Software');
   ...
   //------ Config Cotação ------
   if not ArquivoIni.ValueExists('COTACAO', 'ValorCotacao') then
     ArquivoIni.WriteFloat('COTACAO', 'ValorCotacao', 1);
   if not ArquivoIni.ValueExists('COTACAO', 'EnableCotacao') then
     ArquivoIni.WriteBool('COTACAO', 'EnableCotacao', True);
   if not ArquivoIni.ValueExists('COTACAO', 'DataCotacao') then
     ArquivoIni.WriteDate('COTACAO', 'DataCotacao', now);
   if not ArquivoIni.ValueExists('COTACAO', 'Moeda') then
     ArquivoIni.WriteString('COTACAO', 'Moeda', 'R$');
   ...
   if not ArquivoIni.ValueExists('MULTILOJA', 'BaseLocal') then
     ArquivoIni.WriteString('MULTILOJA', 'BaseLocal',
       extractfiledir(application.Exename) + '\BancoIB\TkAutoIB.GDB');
   ...
   if not ArquivoIni.ValueExists('IBDB', 'TkFcIbServerName') then
     ArquivoIni.WriteString('IBDB', 'TkFcIbServerName', '');
   if not ArquivoIni.ValueExists('IBDB', 'TkFcIbDBPath') then
     ArquivoIni.WriteString('IBDB', 'TkFcIbDBPath',
       extractfiledir(application.Exename) + '\BancoIB\TkAutoIB.GDB');
   if not ArquivoIni.ValueExists('IBDB', 'TkFcIbScriptPath') then
     ArquivoIni.WriteString('IBDB', 'TkFcIbScriptPath',
       extractfiledir(application.Exename) + '\BancoIB\ScriptTkAutoIBS.sql');

   //------ Config LOG ------
   if not ArquivoIni.ValueExists('LOG', 'EnableLog') then
     ArquivoIni.WriteBool('LOG', 'EnableLog', True);
   if not ArquivoIni.ValueExists('LOG', 'TkFcLogFileDir') then
     ArquivoIni.WriteString('LOG', 'TkFcLogFileDir',
       extractfiledir(application.Exename) + '\LOGDir');
   if not ArquivoIni.ValueExists('LOG', 'TkFcLogFilePath') then
     ArquivoIni.WriteString('LOG', 'TkFcLogFilePath',
       extractfiledir(application.Exename) + '\LOGDir\TkFC' + RemoveInvalid('/',
       copy(datetostr(now), 3,length(datetostr(now)))) + '.log');

   //------ Config HelpFile ------
   if not ArquivoIni.ValueExists('HELP', 'EnableHelpFile') then
     ArquivoIni.WriteBool('HELP', 'EnableHelpFile', True);
   if not ArquivoIni.ValueExists('HELP', 'TkFcHelpFilePath') then
     ArquivoIni.WriteString('HELP', 'TkFcHelpFilePath',
       extractfiledir(application.Exename) + '\AJUDA TKFC.chm');
 except
...
begin
 try
   ...
   ArquivoIni := TIniFile.Create(Path);
   HelpFilePath := ArquivoIni.ReadString('HELP', 'TkFcHelpFilePath',
     extractfiledir(application.Exename) + '\AJUDA TKFC.chm');
     ...
     if application.messagebox(PChar('Não foi possível encontrar Arquivo de Ajuda:' +
      ...
      begin
       ...
       if MyOpenDialog.Execute then
        begin
         ArquivoIni.WriteString('HELP', 'TkFcHelpFilePath', MyOpenDialog.FileName);
