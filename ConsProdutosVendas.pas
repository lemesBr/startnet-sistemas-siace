...
uses
  ...
  Mask, Buttons, RxLookup, BaseGrid,  DBAdvGrid, FireDAC.Stan.Intf,
  FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf,
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet, FireDAC.Comp.Client, sBitBtn,
  AdvUtil, AdvObj, AdvGrid, AdvGlowButton;
...
procedure TFormConsProdutosVendas.BuscaValorEmTodosCamposTabela(Tabela, Busca : String);
var
  Query : TFDQuery;
begin
...
begin
       ...
       DM.Sds_produtos_C.DataSet.CommandText:= 'select * from PRODUTOS WHERE ATIVO = ''A'' order by DESCRICAO ASC';
       dm.SDS_PRODUTOS_C.Active := True;
       DM.DTS_PRODUTOS_C.DataSet.Filtered := False;
       DM.DTS_PRODUTOS_C.DataSet.Filter:= 'codigo_barras LIKE ' +QuotedStr('%'+'-1'+'%');
       DM.DTS_PRODUTOS_C.DataSet.Filtered:= True;
end else
begin
  ...
  BEGIN
      ...
      xDBGrid1.SetFocus;
        ...
        IF Edit2.Text = 'LETRAS' then
         begin
           //ShowMessage('É Necessario digitar algo para pesquisar');
          dm.SDS_PRODUTOS_C.Active := false;
          dm.SDS_PRODUTOS_C.DataSet.CommandText:= 'select * from PRODUTOS where ATIVO ='+'''A'''+ ' AND DESCRICAO like ' + #39 + '%' + Edit1.Text + '%' + #39+'' +'ORDER BY DESCRICAO';
          dm.SDS_PRODUTOS_C.Active := True;
          XDBGRID1.SetFocus;
         end;
         ...
         BEGIN
         dm.SDS_PRODUTOS_C.Active := false;
         dm.SDS_PRODUTOS_C.DataSet.CommandText:= 'select * from PRODUTOS where ATIVO ='+'''A'''+ ' AND PROCURA like ' + #39 + '%' + RemoveChar(Edit1.Text) + '%' + #39+'ORDER BY DESCRICAO';
         dm.SDS_PRODUTOS_C.Active := True;
         Label1.Caption := 'N° Produtos Ativos Fitrados: ' + IntToStr(dm.SDS_PRODUTOS_C.RecordCount);

            If dm.SDS_PRODUTOS_C.locate('PROCURA', RemoveChar(Edit1.Text),[])=True then
            begin
            XDBGRID1.SetFocus;
            end else
            If not dm.SDS_PRODUTOS_C.locate('PROCURA', RemoveChar(Edit1.Text),[])=True then
            begin
             SHOWMESSAGE('Produto Não Localizado');
             Edit1.SetFocus;
            end;
    ...
    {  if cbxpesquisa.ItemIndex = 4 then      // por Codigo
       ...
       {   end ;
         ...
         dm.SDS_PRODUTOS_c.Active := false;
         DM.Sds_produtos_c.DataSet.CommandText:= 'select * from PRODUTOS where codigo like ' + #39 + '%' + RemoveChar(Edit1.Text) + '%' + #39+'ORDER BY DESCRICAO';
         dm.SDS_PRODUTOS_c.Active := True;
         dm.SDS_PRODUTOS_C.Filtered := false;
         dm.SDS_PRODUTOS_C.Filter:= 'ATIVO ='+QuotedStr (ATIVO);
         dm.SDS_PRODUTOS_C.Filtered := True;
         Label1.Caption := 'N° Produtos Ativos Fitrados: ' + IntToStr(dm.SDS_PRODUTOS_C.RecordCount);
              ...
              If DM.SDS_PRODUTOS_C.locate('codigo', RemoveChar(Edit1.Text),[])=True then
              begin
              XDBGRID1.SetFocus;
              end else
              If not DM.SDS_PRODUTOS_C.locate('codigo', REMOVECHAR(Edit1.Text),[])=True then
              begin
