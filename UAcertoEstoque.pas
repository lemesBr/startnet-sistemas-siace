...
procedure TFormAcertoEstoque.GravaItensColetor(const CodProd : integer;
const Quant : currency; const Valor : currency);
var
  iOrd : integer;
begin
  iOrd:= Ordem;
...
begin
  inherited;
  if MessageDlgAde('Informe o tipo de Acerto (Entradas ou Saidas).',
    MtInformation,[MbYes, MbNo],0, mrNo, True)=Mrno Then
  begin
    lblTipoAcerto.Tag:= 2;
...
begin
  inherited;
  if Application.MessageBox('Confirma a Exclusão do Acerto?', 'Atenção',
...
begin
  ...
  begin
    close;
    CommandText.Clear;
    CommandText.Text:= 'UPDATE PRODUTOS SET VALIDADE = :DATA, '+
    'PRECO_CUSTO =:CUSTO, PRECO_VENDA = :VENDA, PRECO_VENDA2 = :VENDA2 ' +
    'WHERE CODIGO = :CODPRO';
...
begin
  ...
  begin
    Application.MessageBox('O Preço de Venda não pode ser menor ou igual a 0!!!',
     'Atenção', mb_applmodal+MB_ICONWARNING+MB_OK);
    edtVenda.SetFocus;
    Exit;
  end;
  ...
  begin
    Application.MessageBox('O Preço de Venda 2 não pode ser menor que 0!!!',
    'Atenção', mb_applmodal+MB_ICONWARNING+MB_OK);
    edtVenda2.SetFocus;
    Exit;
  end;
  ...
  if EdtCodPro.Text = '' then
  begin
    Application.MessageBox('É necessário preencher o código do Produto!',
    'Atenção', mb_applmodal+MB_ICONWARNING+MB_OK);
    EdtCodPro.SetFocus;
    Exit;
  end;
  ...
  BuscaItens;
  if lblTipoAcerto.Tag = 1 then
    formprincipal.log(codigo_usuario,'AC_ESTOQUE',QBuscaItensCOD_PRO.Text,
    'ALT +','FDICIONOU EST. DO PRODUTO Nº '+QBuscaItensCOD_PRO.Text +' Nome: '+
    QBuscaItensDESCRICAO.text + ' Qtde '+ QBuscaItensQUANT.text)
  else
    formprincipal.log(codigo_usuario,'AC_ESTOQUE',QBuscaItensCOD_PRO.Text,
    'ALT -','DIMINUIU EST. DO PRODUTO Nº '+QBuscaItensCOD_PRO.Text +' Nome: '+
     QBuscaItensDESCRICAO.text + ' Qtde '+ QBuscaItensQUANT.text);
  ...
  begin
    ...
    begin
      ...
      try
        ...
        BuscaItens;
        formprincipal.log(codigo_usuario,'AC_ESTOQUE',QBuscaItensCOD_PRO.Text,
        'EXCLUI','EXCLUSAO ACERTO DO PRODUTO Nº '+QBuscaItensCOD_PRO.Text +
        ' Nome: '+ QBuscaItensDESCRICAO.text + 'Qtde '+ QBuscaItensQUANT.text);
      end;
    end;
  end
  else
    Application.MessageBox('Você não possue item para que possa excluir!!!',
    'Atenção', mb_applmodal+MB_ICONWARNING+MB_OK);
end;
...
begin
  ...
  if OP.Execute then
  begin
    if Application.MessageBox('Confirma a Importação?', 'Atenção',
    mb_applmodal+mb_iconquestion+mb_yesno+mb_defbutton1) = 6 then
    begin
...
begin
  ...
  begin
    CorSaida(Sender);
    EdtNomeLocal.Text:= consulta('locais_estoque', EdtCodLocal, 'codigo',
