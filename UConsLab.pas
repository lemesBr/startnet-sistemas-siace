...
implementation
Uses ModulodeDados, Ubibli1, UAlteraPreco, UCadLab;
...
begin
  ...
  dm.GravaIni('siace.ini', 'PosicaoCombobox', 'FrmConsLab.ComboCons', inttostr(ComboCons.itemIndex));
  Action:= caFree;
end;
...
begin
  ...
  case tag of
    0,3: begin
            if tag = 3 then
               Application.CreateForm(TFrmCadLab, FrmCadLab);
            with FrmCadLab do
               begin
                  tag:= 1;
                  limpaedit(FrmCadLab);
...
begin
  ...
  try
    ComboCons.ItemIndex:= strtoint(dm.LeINI('siace.ini', 'PosicaoCombobox', 'FrmConsLab.ComboCons'));
  except
