...
uses
  ...
  FireDAC.Phys.IB, FireDAC.Comp.Client, IBUpdateSQL, DB, ACBrEAD,
  ...
  IBX.IBCustomDataSet, ACBrIntegrador, ACBrPosPrinter, ACBrNFeDANFEFR,
  frxExportBaseDialog, ACBrDFeConfiguracoes, pcnAuxiliar,
  ACBrDFeSSL, ACBrNFeDANFeESCPOS, System.Math, blcksock,
  ACBrDANFCeFortesFrA4,  ACBrNFeDANFeRLClass, vcl.Menus ;
...
type
    ...
    sds_LyBancosLOCAL_PAGAMENTO: TStringField;
...
//    Procedure CloseIBArray(IBSender: array of TIBCustomDataset);
    ...
    procedure sds_LyBancosNewRecord(DataSet: TDataSet);
...
uses Uerro, Vendas, ufrmStatus, Ubibli1, UDialog, clientes, wcrypt2,synacode, pcnNFe,
    ACBrDFeOpenSSL, pcnNFeRTXT;
...
procedure TDM.sds_LyBancosNewRecord(DataSet: TDataSet);
VAR
  Caminho: string;
begin

  Caminho := ExtractFilePath(Application.ExeName) + 'boletos\Remessa';

  if DirectoryExists(Caminho) then
    TFDQuery(dm.Dts_lyBancos.DataSet).FieldByName('DIRENVIA').AsString :=  ExtractFilePath(Application.ExeName) + 'boletos\Remessa';

  if DirectoryExists(Caminho) then
    TFDQuery(DM.Dts_lyBancos.DataSet).FieldByName('DIRRECEBE').AsString :=  ExtractFilePath(Application.ExeName) + 'boletos\Retorno';

  {if DirectoryExists(Caminho) then
    TFDQuery(dtsDefault.DataSet).FieldByName('DIRRECEBEBKP').AsString :=
      ExtractFilePath(Application.ExeName) + 'Boleto\Backup';     }

  TFDQuery(dm.Dts_lyBancos.DataSet).FieldByName('LOCAL_PAGAMENTO').AsString := 'PAGAR EM QUALQUER BANCO OU NAS CASAS LOTERICAS';

  TFDQuery(dm.Dts_lyBancos.DataSet).FieldByName('CNAB').AsString := 'CNAB 400';

  TFDQuery(dm.Dts_lyBancos.DataSet).FieldByName('VLROUTRASDEPESAS').AsCurrency := 0.00;
end;
...
//   try
...

  {
    ...
    SQLC.Connected    := true;
     ...
     // Coneccao.Database := '';
     //   Coneccao.Params.Values['PORT']      :=trim(LeINI('siace.ini', 'DADOS', 'PORTA') );
     //  Coneccao.Params.Values['PROTOCOL']  :=trim(LeINI('siace.ini', 'DADOS', 'PROTOCOLO') );
     //  Coneccao.Params.Values['SERVER']    :=trim(LeINI('siace.ini', 'DADOS', 'SERVIDOR') );
  ...
  //   Coneccao.Params.Values['DATABASE'] := trim(LeINI('siace.ini', 'DADOS', 'DATABASE'));
    // Coneccao.Connected    := true;
     ...
     {  if uppercase(trim(LeINI('Siace.ini', 'DADOS', 'SPED'))) = 'TRUE' then
        begin
          ...
          try
            try
              ...
              if qrsql.fieldbyname('sped').asinteger = 0 then
                shellexecute(hwnd_desktop,'open', pchar('C:\StartNet_Sistemas\efdsiace.exe'), nil, nil, sw_shownormal);
            except
              shellexecute(hwnd_desktop,'open', pchar('C:\StartNet_Sistemas\efdsiace.exe'), nil, nil, sw_shownormal);
            end;
          finally
