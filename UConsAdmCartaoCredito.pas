...
implementation
Uses ModulodeDados, Ubibli1, URelContasReceberCartao, UCadAdmCartaoCredito,
  UConsCTRAdm, URecAdm;
...
begin
  ...
  dm.GravaIni('siace.ini', 'PosicaoCombobox', 'FrmConsAdmCartaoCredito.ComboCons', inttostr(ComboCons.itemIndex));
  Action:= caFree;
end;
  ...
  {6: begin
         with FrmDadosAdm do
            begin
               ...
               EdtNomeBandeira.TEXT := dm.QAdmCartaoDESCRICAO.AsString;
...
begin
  ...
  try
    ComboCons.ItemIndex:= strtoint(dm.LeINI('siace.ini', 'PosicaoCombobox', 'FrmConsAdmCartaoCredito.ComboCons'));
  except
