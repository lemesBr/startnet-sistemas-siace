   ...
   try
    try
       with QAlteraPreco do
          begin
             close;
             Parambyname('PRECO_VENDA').AsCurrency        := edtvalor.value;
             Parambyname('PRECO_PROMOCAO').AsCurrency     := EdtPromocao.value;
             Parambyname('PRECO_VENDA2').AsCurrency       := EdtProgramado.value;
             Parambyname('preco_custo').AsCurrency        := EdtCusto.value;
