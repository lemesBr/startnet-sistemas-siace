...
Uses Ubibli1, UCadConvenio, URelConv, URelRecebimento, URecConvenio,
  ModulodeDados, clientes, CRCrediarioconv;
...
begin
  ...
  dm.GravaIni('SIACE.ini', 'PosicaoCombobox', 'FrmConsConvenio.ComboCons', inttostr(ComboCons.itemIndex));
  Action:= caFree;
end;
...
begin
  ...
  case tag of
   ...
   3:  begin
          with FormCRCrediarioConv do
             begin
                cliente.Text   := dm.QConvCOD_CON.AsString;
                RXLabel5.caption   := dm.QConvCOD_CON.AsString;
                RXLabel6.Caption  := dm.QConvNOME_CON.Value;
                btnok.SetFocus;
             end;
          close;
       end;  
...
begin
  ...
  try
    ComboCons.ItemIndex:= strtoint(dm.LeINI('siace.ini', 'PosicaoCombobox', 'FrmConsConvenio.ComboCons'));
  except
    ComboCons.ItemIndex:= 0;
  end;
end;

procedure TFrmConsConvenio.BtnSairClick(Sender: TObject);
begin
  inherited;
CLOSE;
end;
