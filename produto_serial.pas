...
begin
 { if serial_entrada = true then
  begin
    ...
    qrserial_produto.sql.add('select * from PRODUTOS_SERIAL where coditem = ''' + frmPRODUTO_ENTRADA.qrENTRADA_ITEM.fieldbyname('codigo').asstring + ''' order by codigo');
  ...
  begin
    ...
    qrserial_produto.close;
    qrserial_produto.sql.clear;
    qrserial_produto.sql.add('select * from PRODUTOS_SERIAL where coditem = ''' + frmcompra.qrnota_item.fieldbyname('codigo').asstring + ''' order by codigo');
    qrserial_produto.Open;
    RTOTAL.Value := 0;
    rdigitado.Value := 0;
    Rdiferenca.Value := 0;
    RTOTAL.Value := frmcompra.qrnota_item.fieldbyname('quantidade').asfloat;
  end;
 ...
 { if serial_entrada = true then
  begin
   ...
   DM.ProxCod.Open;
    ...
    qrserial_produto.FieldByName('codigo').asinteger := DM.ProxCodN_CODIGO.ASINTEGER + 1;
    ...
    qrserial_produto.fieldbyname('codnota').asstring := DM.qrentrada_produto.fieldbyname('codigo').asstring;
    qrserial_produto.fieldbyname('NUMERONOTA').asstring := DM.qrentrada_produto.fieldbyname('NUMERO').asstring;
    qrserial_produto.fieldbyname('codFORNECEDOR').asstring := DM.qrentrada_produto.fieldbyname('codFORNECEDOR').asstring;
    ...
    qrserial_produto.fieldbyname('DATACOMPRA').asstring := DM.qrentrada_produto.fieldbyname('DATA').asstring; ;
  end
  else  }
  begin
   dm.ProxCod.Close;
