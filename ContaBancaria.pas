...
uses
  ...
  TFormContaBancaria = class(TForm)
    ...
    procedure econta1Exit(Sender: TObject);
    procedure econta1Enter(Sender: TObject);
...
uses ModulodeDados, Z_RotinasGerais, Principal, Acesso, ConsUnidades,
  ConsMarcas, Marcas, ConsContaBancariapas, ConsPlanoContas;
...
// Try
      begin
        ...
        //  dm.SDS_Conta_BancariaCOD_PLANO.ASSTRING := econta.Text;
      end;
{   Except
      MsgErro.Text:= 'Ocorreu um Erro. Reinicie o Sistema e tente novamente !!!';
   MsgErro.ShowModal;
...
procedure TFormContaBancaria.blocregiaoClick(Sender: TObject);
begin
 FormConsPlanoContas:=TFormConsPlanoContas.Create(self);
     FormConsPlanoContas.ShowModal;
      dm.SDS_Conta_Bancaria.Edit;
       dm.SDS_Conta_BancariaCOD_PLANO.ASSTRING := DM.SDS_PlContasCODIGO.Text;
       econta.Text := DM.SDS_PLCONTASCODIGO.TEXT;
       enomeconta.Text := DM.SDS_PlCONTASnome.TEXT;
end;

procedure TFormContaBancaria.econta1Exit(Sender: TObject);
begin
...
procedure TFormContaBancaria.econta1Enter(Sender: TObject);
begin
CorEntrada(sender);
end;

procedure TFormContaBancaria.econtaEnter(Sender: TObject);
begin
CorEntrada(sender);
end;

procedure TFormContaBancaria.DBNavigatorClick(Sender: TObject;
  Button: TNavigateBtn);
begin
 if econta.Text > '' then
  begin
...
procedure TFormContaBancaria.econtaKeyPress(Sender: TObject;
  var Key: Char);
begin
 if key = #13 then
 begin
