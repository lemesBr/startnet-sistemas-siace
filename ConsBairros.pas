...
uses
  ...
  Dialogs, Grids, DBGrids, StdCtrls, RXCtrls, ExtCtrls, DB, Vcl.DBCtrls,
  sDBLookupComboBox, sLabel;
...
DM.Sds_BAIRROS.sql.clear;
DM.Sds_BAIRROS.sql.add('SELECT * FROM BAIRRO order by DESCRICAO asc');
...
DM.Sds_BAIRROS.Active := FALSE;
DM.Sds_BAIRROS.sql.clear;
DM.Sds_BAIRROS.sql.add('SELECT * FROM BAIRRO WHERE DESCRICAO LIKE ' + #39 + '%' + Edit1.Text + '%' + #39+'order by DESCRICAO asc');
DM.Sds_BAIRROS.Active := TRUE;
end;
end;
