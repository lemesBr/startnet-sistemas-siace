object FPatioMadeireira: TFPatioMadeireira
  Left = 708
  Top = 140
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Cadastro de Patios'
  ClientHeight = 453
  ClientWidth = 477
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object pgcPatio: TPageControl
    Left = 0
    Top = 0
    Width = 477
    Height = 453
    ActivePage = tbGrid
    Align = alClient
    TabOrder = 0
    object tbGrid: TTabSheet
      Caption = 'tbGrid'
      TabVisible = False
      object Grid: TDBGrid
        Left = 0
        Top = 41
        Width = 469
        Height = 402
        Align = alClient
        DataSource = dsPatio
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgRowSelect, dgCancelOnExit]
        ParentFont = False
        TabOrder = 1
        TitleFont.Charset = ANSI_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -16
        TitleFont.Name = 'Calibri'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'ID'
            Visible = False
          end
          item
            Expanded = False
            FieldName = 'NOME'
            Title.Caption = 'Nome'
            Width = 400
            Visible = True
          end>
      end
      object pnlNav: TPanel
        Left = 0
        Top = 0
        Width = 469
        Height = 41
        Align = alTop
        BevelOuter = bvNone
        TabOrder = 0
        object spbIncluir: TSpeedButton
          Left = 7
          Top = 3
          Width = 120
          Height = 33
          Caption = 'Incluir [F2]'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Calibri'
          Font.Style = []
          Glyph.Data = {
            
            FF00FFFFFF00FFFFFF00}
          ParentFont = False
          OnClick = spbIncluirClick
        end
        object spbAlterar: TSpeedButton
          Left = 131
          Top = 3
          Width = 120
          Height = 33
          Caption = 'Alterar [F3]'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Calibri'
          Font.Style = []
          Glyph.Data = {
            
            FF00FFFFFF00FFFFFF00}
          ParentFont = False
          OnClick = spbAlterarClick
        end
        object spbExcluir: TSpeedButton
          Left = 255
          Top = 3
          Width = 120
          Height = 33
          Caption = 'Excluir [F4]'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Calibri'
          Font.Style = []
          Glyph.Data = {
            
            FF00FFFFFF00FFFFFF00}
          ParentFont = False
          OnClick = spbExcluirClick
        end
      end
    end
    object tbEdit: TTabSheet
      Caption = 'tbEdit'
      ImageIndex = 1
      TabVisible = False
      object Bevel2: TBevel
        Left = 0
        Top = 0
        Width = 465
        Height = 41
        Shape = bsFrame
      end
      object Label1: TLabel
        Left = 13
        Top = 10
        Width = 51
        Height = 16
        Caption = 'Esp'#233'cie:'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
      end
      object pnlFooterEdit: TPanel
        Left = 0
        Top = 394
        Width = 469
        Height = 49
        Align = alBottom
        BevelOuter = bvNone
        TabOrder = 0
        object spbCancelar: TSpeedButton
          Left = 10
          Top = 10
          Width = 138
          Height = 33
          Caption = 'Cancelar [F11]'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Calibri'
          Font.Style = []
          Glyph.Data = {
            
            FF00FFFFFF00FFFFFF00}
          ParentFont = False
          OnClick = spbCancelarClick
        end
        object spbSalvar: TSpeedButton
          Left = 336
          Top = 10
          Width = 120
          Height = 33
          Caption = 'Salvar [F12]'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Calibri'
          Font.Style = []
          Glyph.Data = {
            
            FF00FFFFFF00FFFFFF00}
          ParentFont = False
          OnClick = spbSalvarClick
        end
        object Bevel1: TBevel
          Left = 0
          Top = 0
          Width = 469
          Height = 1
          Align = alTop
          Shape = bsTopLine
        end
      end
      object edtPatio: TDBEdit
        Left = 72
        Top = 8
        Width = 377
        Height = 24
        DataField = 'NOME'
        DataSource = dsPatio
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
      end
    end
  end
  object dsPatio: TDataSource
    DataSet = DMMovimentacao.qrMD_PATIO
    Left = 224
    Top = 224
  end
end
