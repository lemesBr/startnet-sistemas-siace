...
uses
  ...
  IBDatabase, DB, IBCustomDataSet, IBQuery, AlignEdit,
  FMTBcd, SqlExpr, RxToolEdit, RxCurrEdit;
         ...
         try
           try
                            ...
                            begin
                               ...
                               if trim(EdtCodConta.Text) <> '' then
                                 ...
                                 { SPC_MovimentoBancario.Params[1].AsDate:= EdtData.Date;
                                  ...
                                  SPC_MovimentoBancario.Params[11].Value:= edttotalliq.value;
                                  SPC_MovimentoBancario.Params[12].Value:= 15;
                                  SPC_MovimentoBancario.Params[13].Value:= dm.sds_configuracoes.fieldbyname('PLANO_RECEBIMENTO_CREDIARIO').asstring;
            ...
            if dm.IBTransaction.Active then
                                    ...
                                    qrcaixa_mov.parambyname('valor_entrada').asfloat :=  edttotalliq.value;;
                                    qrcaixa_mov.parambyname('valor').asfloat :=  edttotalliq.value;;
                                    ...
                                    qrcaixa_mov.parambyname('tipo_mov').asINTEGER := 15; // VENDA PRAZO
                                    qrcaixa_mov.parambyname('historico').asstring := 'Depósito Ref Credito Cartão Na conta nº ' +EdtNomeCentroCusto.Text + '-'+ EditDigito.Text;
                                    qrcaixa_mov.parambyname('tipo').AsString  := 'B';
                                    qrcaixa_mov.parambyname('ORIGEM').AsString  := 'CAR';
                                    qrcaixa_mov.parambyname('CODIGO_CC').AsString  := DM.SDS_Conta_BancariaCODIGO.AsString;
