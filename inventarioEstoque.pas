...
uses
  ...
  RXDBCtrl, XDBGrids, Mask, rxToolEdit, AlignEdit, Buttons,
  Menus, AdvGlowButton, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf,
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.Client,
  FireDAC.Comp.DataSet, NxPageControl;
...
type
  TFormInventarioEstoque = class(TForm)
    ...
    QBuscaItensCOD_BARRAS: TStringField;
    QBuscaItensREFERNCIA: TStringField;
    QBuscaItensDESCRICAO_NOVA: TStringField;
    QBuscaItensQUANTIDADE: TFMTBCDField;
    QBuscaItensQUANT_FRACIONADA: TFMTBCDField;
    ...
    QBuscaItensSTATUS: TStringField;
...
uses Principal, Z_RotinasGerais, ModulodeDadosConsultas,
  ConsProdutos, IniFiles, UConsLocaisEstoque, Ubibli1, ModulodeDados;
...
//Try
  ...
  {if not panel5.Enabled then
   ...
   dm.ProxCod.SQL.Add('select max(cod_inventario) as N_CODIGO FROM inventario_estoque');
   DM.ProxCod.Open;
     ...
     begin
        ...
        //  try
            with QInsInv do
               begin
                  close;
                  cod_inv.Text:= INTTOSTR( DM.ProxCodN_CODIGO.ASINTEGER + 1 );// inttostr(prxcod('inventario_estoque', 'cod_inventario', dm.IBTransaction, dm.QConsulta));
...
begin
    if ComboEdit1.Text > '' then
     ...
     DM.Sds_produtos.sql.add('select * from PRODUTOS where CODIGO_BARRAS like ' + #39 + '%' +  ComboEdit1.Text + '%' + #39+'ORDER BY DESCRICAO');
     DM.Sds_produtos.Active := True;
...
begin
 ...
 IF XDBGRID1.COLUMNS.Grid.Fields[5].Value < 1 then
  begin
   ...
   IF xDBGRID1.COLUMNS.Grid.Fields[9].TEXT = '' THEN
   begin
     xDBGRID1.COLUMNS.Grid.Fields[9].VALUE := 0;
   end;
end;
...
begin
           ...
           begin
            ...
            SPC_ATUALIZA.ParamByname('Quant').Value         := QBuscaItensQUANTIDADE.AsFloat;
            SPC_ATUALIZA.ParamByname('CUSTO').Value         := QBuscaItensPRECO_CUSTO.asfloat;
            SPC_ATUALIZA.ParamByname('VENDA').Value         := QBuscaItensPRECO_VENDA.AsFloat;
            SPC_ATUALIZA.ParamByname('VENDA2').Value        := QBuscaItensPRECO_VENDA2.AsFloat;
            ...
            SPC_ATUALIZA.ParamByname('Fracao').Value        := QBuscaItensFRACAO.AsFloat;
            SPC_ATUALIZA.ParamByname('Quant_Fracao').Value  := QBuscaItensQUANT_FRACIONADA.AsFloat;
