...
type
  Tfrmlista_caixa1 = class(TForm)
    ...
    Bevel1: TBevel;
    Bevel2: TBevel;
    spbImprimir: TAdvGlowButton;
    qrcaixaCODIGO: TIntegerField;
    qrcaixaDATA: TDateField;
    qrcaixaCOMPENSADO: TStringField;
    qrcaixaDOCUMENTO_LANC: TStringField;
    qrcaixaCOD_EMPRESA: TIntegerField;
    qrcaixaDB_CR: TStringField;
    qrcaixaVALOR_ENTRADA: TFMTBCDField;
    qrcaixaVALOR_SAIDA: TFMTBCDField;
    qrcaixaVALOR: TFMTBCDField;
    qrresumoSUM: TFMTBCDField;
...
var
  frmlista_caixa1: Tfrmlista_caixa1;
...
uses  principal, ModulodeDados, ConsPlanoContas, ConsUsuarios, xloc_caixa;

{$R *.dfm}

procedure Tfrmlista_caixa1.combo_caixaChange(Sender: TObject);
begin
...
procedure Tfrmlista_caixa1.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action := cafree;
end;

procedure Tfrmlista_caixa1.FormShow(Sender: TObject);
BEGIN
...
procedure Tfrmlista_caixa1.DateEdit1KeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl,0,0);
end;

procedure Tfrmlista_caixa1.bimprimirClick(Sender: TObject);
var CAIXA, conta: string;
begin

end;

procedure Tfrmlista_caixa1.qrcaixaCalcFields(DataSet: TDataSet);
begin
...
procedure Tfrmlista_caixa1.combo_contaChange(Sender: TObject);
begin
...
procedure Tfrmlista_caixa1.spbImprimirClick(Sender: TObject);
var
  CAIXA, conta: string;
begin
  DM.qrrelatorio.open;
...
//  DM.QRRELATORIO.FIELDBYNAME('LINHA2').ASSTRING := 'PERÍODO : '+DATEEDIT1.TEXT+' A '+DATEEDIT2.TEXT;
  ...
  DM.QRRELATORIO.FIELDBYNAME('LINHA2').ASSTRING := ' HORA AB:' + DM.sds_caixa.FieldByName('HORA_ABERT').AsString +  ' HORA FECH.:' + DM.sds_caixa.FieldByName('HORA_FECHA').AsString;
  ...
  qrsoma.sql.add('select sum(valor_entrada) total_entrada, sum(valor_saida) total_saida, '+ // total_entrada e total_saida
                '(select sum(VALOR) valor   from v_movimento_diario where tipo_mov = 03 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // sum
                '(select sum(VALOR) valor_1 from v_movimento_diario where tipo_mov = 04 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // sum_1
                '(select sum(VALOR) valor_2 from v_movimento_diario where tipo_mov = 05 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // sum_2
                '(select sum(VALOR) valor_3 from v_movimento_diario where tipo_mov = 06 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // sum_3
                '(select sum(VALOR) valor_4 from v_movimento_diario where tipo_mov = 07 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // sum_4
                '(select sum(VALOR) valor_5 from v_movimento_diario where tipo_mov = 08 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // sum_5
                '(select (sum(VALOR_ENTRADA) - sum(VALOR_SAIDA)) AS valor_6 from v_movimento_diario where tipo_mov = 09 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 6 rec dinheiro
                '(select sum(VALOR) valor_7 from v_movimento_diario where tipo_mov = 10 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 7 rec che av
                '(select sum(VALOR) valor_8 from v_movimento_diario where tipo_mov = 11 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 8 rec che ap
                '(select sum(VALOR) valor_9 from v_movimento_diario where tipo_mov = 12 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 9 rec cartao
                '(select sum(VALOR) valor_10 from v_movimento_diario where tipo_mov = 13 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 10 rec juros
                '(select sum(VALOR) valor_11 from v_movimento_diario where tipo_mov = 14 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 11 rec desconto
                '(select sum(VALOR) valor_12 from v_movimento_diario where tipo_mov = 15 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 12 rec boleto
                '(select sum(VALOR) valor_13 from v_movimento_diario where tipo_mov = 25 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 13 pagar - caixa
                '(select sum(VALOR) valor_14 from v_movimento_diario where tipo_mov = 26 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 14 pagar - banco
                '(select sum(VALOR) valor_15 from v_movimento_diario where tipo_mov = 27 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 15 pagar - terceiro
                '(select sum(VALOR) valor_16 from v_movimento_diario where tipo_mov = 2  and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 16 LANCTO SAIDA
                '(select sum(VALOR) valor_17 from v_movimento_diario where tipo_mov = 17 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 17 OUTRAS SAIDAS
                // servicos
                '(select sum(VALOR) valor_18 from v_movimento_diario where tipo_mov = 18 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // sum_18  - dinheiro
                '(select sum(VALOR) valor_19 from v_movimento_diario where tipo_mov = 19 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // sum_19  - cree
                '(select sum(VALOR) valor_20 from v_movimento_diario where tipo_mov = 20 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // column_20  - cv
                '(select sum(VALOR) valor_21 from v_movimento_diario where tipo_mov = 21 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // column_21  - cp
                '(select sum(VALOR) valor_22 from v_movimento_diario where tipo_mov = 22 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // column_22  - cc
                '(select sum(VALOR) valor_23 from v_movimento_diario where tipo_mov= 23 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // column_23  - cd
                '(select sum(VALOR) valor_24 from v_movimento_diario where tipo_mov = 40 and COD_FECHAMENTO = :COD_F '+caixa+conta+'), '+ // 24 convenios
                '(select sum(VALOR) valor_25 from v_movimento_diario where tipo_mov = 42 and COD_FECHAMENTO = :COD_F '+caixa+conta+') '+ // 24 convenios
                'from v_movimento_diario where  COD_FECHAMENTO = :COD_F '+caixa+conta);

        // qrsoma.Params.ParamByName('datai').asdatetime := dateedit1.date;
       //  qrsoma.Params.ParamByName('dataf').asdatetime := dateedit2.date;

   qrsoma.Params.ParamByName('COD_F').AsInteger:= StrToInt(resultado_pesquisa1);
   qrsoma.open;
  ...
  begin
      ...
      qrcaixa.close;
      qrcaixa.SQL.Clear;
      qrcaixa.sql.add('select * from v_movimento_diario where COD_FECHAMENTO = :COD_F '+caixa+conta+' order by data, codigo asc');
      qrcaixa.Params.ParamByName('COD_F').AsInteger:= StrToInt(resultado_pesquisa1);
    ...
    fxcAIXA.LoadFromFile('rel\f000071.fr3');
    fxcAIXA.ShowReport;
  end;
  ...
  begin
    ...
    fxcAIXA.LoadFromFile('rel\f000071_1.fr3');
    fxcAIXA.ShowReport;
  end;
  ...
  begin
    ...
    qrcaixa.close;
    qrcaixa.SQL.Clear;
    qrcaixa.sql.add('select * from v_movimento_diario where COD_FECHAMENTO = :COD_F '+caixa+conta+' and tipo_mov in (2,17,22,26,27) order by data, codigo');
    qrcaixa.Params.ParamByName('COD_F').AsInteger:= StrToInt(resultado_pesquisa1);
    // qrcaixa.Params.ParamByName('datai').asdatetime := dateedit1.date;
    // qrcaixa.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    // qrcaixa.Params.ParamByName('tp').AsString := 'C';
    ...
    fxcAIXA.LoadFromFile('rel\f000072.fr3');
    fxcAIXA.ShowReport;
  end;
  ...
  begin
    ...
    qrcaixa.close;
    qrcaixa.SQL.Clear;
    qrcaixa.sql.add('select * from v_movimento_diario where COD_FECHAMENTO = :COD_F '+caixa+conta+' and tipo_mov in (3,5,8,9,10,11,15,16) order by data, codigo');
    qrcaixa.Params.ParamByName('COD_F').AsInteger:= StrToInt(resultado_pesquisa1);
    //  qrcaixa.Params.ParamByName('datai').asdatetime := dateedit1.date;
    // qrcaixa.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    // qrcaixa.Params.ParamByName('tp').AsString := 'C';
    ...
    DM.qrrelatorio.fieldbyname('LINHA1').asstring := 'RESUMO DE RECEBIMENTOS - ENTRADAS';
    fxcAIXA.LoadFromFile('rel\f000073.fr3');
    fxcAIXA.ShowReport;
  end;
