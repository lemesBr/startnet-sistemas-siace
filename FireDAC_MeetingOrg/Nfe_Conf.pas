...
uses IniFiles, ShellAPI,
  ...
  pcnNFeW, pcnNFeRTXT, Mylabel, Mask, ACBrEAD, DBCtrls, ACBrDFeUtil;
...
begin
 { with DM.ACBrNFe1.NotasFiscais.Add.NFe do
   begin
     Ide.cNF       := DM.SDS_NFENr_nota.text; // StrToInt(NumNFe); //Caso não seja preenchido será gerado um número aleatório pelo componente
...
begin
{ if Application.MESSAGEBOX('Confirma a Importação dos Dados da Empresa, Serão Substituidos os dados Atuais?', 'confirmar', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON1) = ID_YES then
 begin
...
{  codOrgao := copy(Chave,1,2);
  ...
  if not(InputQuery('WebServices Carta de Correção', 'Correção a ser considerada', data)) then
     exit;
