...
uses
  ...
  Dialogs, StdCtrls, RXCtrls, Grids, aDvObj, BaseGrid, DBAdvGrid,
...
uses Z_RotinasGerais, ModuleDados1 , ModulodeDados;
...
begin
if DM1.Sds_valeTroco.Locate('VALOR_VALETROCO',Edit1.Text,[lopartialkey]) = True then
begin
  xDBGrid1.SetFocus;
end else
if not DM1.Sds_valeTroco.Locate('VALOR_VALETROCO',Edit1.Text,[lopartialkey]) = True then
begin
...
procedure TFormConsValetroco.SpeedButton1Click(Sender: TObject);
begin
DM1.Sds_valeTroco.Active:=False;
DM1.Sds_valeTroco.DataSet.CommandText := 'select * from VALETROCO order by VALOR_VALETROCO asc';
DM1.Sds_valeTroco.Active:=true;
end;

procedure TFormConsValetroco.SpeedButton2Click(Sender: TObject);
begin
confirma_vale := true;
close;
end;

procedure TFormConsValetroco.SpeedButton3Click(Sender: TObject);
begin
confirma_vale := false;
Close;
end;
