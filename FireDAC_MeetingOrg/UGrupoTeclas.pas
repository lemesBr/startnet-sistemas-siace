...
uses
  ...
  Dialogs, StdCtrls, CurvyControls, Mask, RzEdit, DB, IBCustomDataSet, IBQuery,
  IBDatabase, rxToolEdit, rxCurrEdit;
      ...
      begin
         ...
         with TProdutos( Produtos.Add ) do
            begin
               ...
               if TRTeclas.InTransaction then
                  TRTeclas.Commit;
               ...
               with QConsulta do
                  begin
                     Close;
                     SQL.Clear;
                     SQL.Text := 'select P.PRECO_VENDA, P.GRUPO_TRIBUTACAO, P.CODIGO_BARRAS, P.DESCRICAO,' +
                                 ' U.ABREVIATURA from produtos P' +
                                 ' inner join unidades U' +
                                 ' on ( U.codigo = p.unidade )' +
                                 ' where P.codigo =:codpro';
                     ParamByName( 'codpro' ).AsInteger := CodigoProduto;
                     Open;
                     if not IsEmpty then
                        Valor := FieldByName( 'PRECO_VENDA' ).AsCurrency
                     else
                        Valor := 0;
                     CodICMS := FieldByName( 'GRUPO_TRIBUTACAO' ).AsInteger;
                     Unidade := FieldByName( 'ABREVIATURA' ).AsString;
                     DescCupom :=  FieldByName( 'DESCRICAO' ).AsString;
...
begin
   ...
   with QConsulta do
       begin
          Close;
          Sql.Clear;
          Sql.Text := 'Select T.DESCRICAO, P.descricao as DESC_CUPOM, TD.* from GRUPO_TECLAS_DETALHES TD ' +
                      ' inner join GRUPO_TECLAS T ' +
                      ' on ( T.TECLA = TD.TECLA ) ' +
                      ' INNER JOIN PRODUTOS P ' +
                      ' ON ( P.codigo = TD.cod_pro )' +
