...
uses
  Windows, Vcl.Forms, Messages, SysUtils, Variants, Classes, Graphics, Controls,
  ...
  if Application.MESSAGEBOX('Confirma o Extorno da conta paga?', 'Confirmar', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON1) = ID_YES then
    begin
      ...
      begin
            ...
            begin
             with Application do
                begin
                  NormalizeTopMosts;
                  MessageDlg('Esta conta foi baixada pelo caixa ' + frmcontaspagar.qrcontaspagar.FieldByName('codcaixa').text +
                  ' Apenas ele tem autorização para extorná-la!', mtError, [mbOK], 0);   // [smbOK]
                  RestoreTopMosts;
                end;
              //  application.messagebox('Esta conta foi baixada pelo caixa ' + frmcontaspagar.qrcontaspagar.FieldByName('codcaixa').text +
             //   '. Apenas ele tem autorização para extorná-la!', 'Atenção', mb_ok + MB_ICONWARNING);
            end;
      end
      else
      begin
        text := ('O pagamento desta conta foi lançFDo no caixa do dia: ' +
          frmcontaspagar.qrcontaspagar.FieldByName('data_pagamento').asstring +
          '. Não será possível fazer um lançamento de estorno no caixa atual! Deseja prosseguir com o estorno?');
       //  MessageDlg(texto, mb_yesno + [mb_i],0) = idyes
       if MessageDlg(text, mtConfirmation, [mbYes, mbNo], 0, mbYes) = mrYes then
        begin
                    dm.ProxCod.Close;
                    dm.ProxCod.SQL.Clear;
                    dm.ProxCod.SQL.Add('select max(codigo) as N_CODIGO FROM MOVIMENTO_DIARIO');
     ...
     formprincipal.log(codigo_usuario,'C.PAGAR',FRMCONTASPAGAR.qrcontaspagar.fieldbyname('documento').asstring,'EXTORNOU',' Estornou Conta a Pagar Conta Nr: ' +
      frmcontaspagar.qrcontaspagar.fieldbyname('documento').asstring + ' Fornecedor: ' +  frmcontaspagar.qrcontaspagar.fieldbyname('fornecedor').asstring);
    end;
  end;
end;
end;
