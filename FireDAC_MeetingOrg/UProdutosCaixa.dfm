object FormProdutosCaixa: TFormProdutosCaixa
  Left = 373
  Top = 218
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Produtos Embalagem'
  ClientHeight = 345
  ClientWidth = 738
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object StatusBar1: TStatusBar
    Left = 0
    Top = 326
    Width = 738
    Height = 19
    Panels = <>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 738
    Height = 326
    Align = alClient
    Color = 16710131
    TabOrder = 1
    object Label1: TLabel
      Left = 8
      Top = 16
      Width = 59
      Height = 13
      Caption = 'Cod.Produto'
    end
    object Label2: TLabel
      Left = 208
      Top = 16
      Width = 103
      Height = 13
      Caption = 'Descri'#231#227'o do Produto'
    end
    object Label12: TLabel
      Left = 586
      Top = 16
      Width = 46
      Height = 13
      Caption = 'Quant/Cx'
    end
    object Label3: TLabel
      Left = 87
      Top = 15
      Width = 81
      Height = 13
      Caption = 'Codigo de Barras'
    end
    object Label4: TLabel
      Left = 657
      Top = 16
      Width = 62
      Height = 13
      Caption = 'Pre'#231'o Venda'
    end
    object Label5: TLabel
      Left = 517
      Top = 16
      Width = 40
      Height = 13
      Caption = 'Unidade'
    end
    object spbIncluir: TSpeedButton
      Left = 8
      Top = 289
      Width = 120
      Height = 33
      Action = ActionIncluir
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00}
      ParentFont = False
    end
    object spbAlterar: TSpeedButton
      Left = 132
      Top = 289
      Width = 120
      Height = 33
      Action = ActionAlterar
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00}
      ParentFont = False
    end
    object spbExcluir: TSpeedButton
      Left = 256
      Top = 289
      Width = 120
      Height = 33
      Action = ActionExcluir
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00}
      ParentFont = False
    end
    object spbSalvar: TSpeedButton
      Left = 458
      Top = 289
      Width = 120
      Height = 33
      Action = ActionSalvar
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00}
      ParentFont = False
    end
    object spbCancelar: TSpeedButton
      Left = 583
      Top = 289
      Width = 138
      Height = 33
      Action = ActionCancelar
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlack
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00}
      ParentFont = False
    end
    object EdtCodPro: TAlignEdit
      Left = 8
      Top = 32
      Width = 73
      Height = 21
      Hint = 'Pressione F2 para Consultar ou F3 para Cadastrar'
      Alignment = taRightJustify
      Enabled = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
    end
    object EdtNomePro: TEdit
      Left = 208
      Top = 32
      Width = 305
      Height = 21
      CharCase = ecUpperCase
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
    end
    object EdtQuant: TCurrencyEdit
      Left = 584
      Top = 32
      Width = 65
      Height = 21
      AutoSize = False
      CheckOnExit = True
      DecimalPlaces = 3
      DisplayFormat = ',#0.000'
      FormatOnEditing = True
      TabOrder = 4
      ZeroEmpty = False
    end
    object DBGrid1: TDBGrid
      Left = 8
      Top = 64
      Width = 713
      Height = 225
      TabStop = False
      DataSource = DSProd
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      ReadOnly = True
      TabOrder = 6
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CODIGO_BARRAS'
          Title.Caption = 'CODIGO BARRAS'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCRICAO'
          Title.Caption = 'DESCRI'#199#195'O'
          Width = 303
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UNID'
          Title.Caption = 'UNIDADE'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QUANT_VENDA'
          Title.Caption = 'QUANT CAIXA'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_VENDA'
          Title.Caption = 'PRE'#199'O VENDA'
          Width = 91
          Visible = True
        end>
    end
    object EdtPreco: TCurrencyEdit
      Left = 656
      Top = 32
      Width = 65
      Height = 21
      AutoSize = False
      CheckOnExit = True
      DecimalPlaces = 3
      DisplayFormat = ',#0.000'
      FormatOnEditing = True
      TabOrder = 5
      ZeroEmpty = False
    end
    object EdtCodBar: TAlignEdit
      Left = 88
      Top = 32
      Width = 114
      Height = 21
      Hint = 'Pressione F2 para Consultar ou F3 para Cadastrar'
      Alignment = taRightJustify
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnEnter = EdtCodProEnter
      OnExit = EdtCodProExit
      OnKeyPress = EdtCodProKeyPress
    end
    object DBunidade: TDBLookupComboBox
      Left = 520
      Top = 32
      Width = 65
      Height = 21
      Hint = 'Unidade de Medida'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      KeyField = 'CODIGO'
      ListField = 'ABREVIATURA'
      ListFieldIndex = 1
      ListSource = DM.DTS_Unidades
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
    end
  end
  object QProd: TIBQuery
    Database = DM.IBDatabase
    Transaction = IBTRProd
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT * FROM PRODUTOS_FARDO'
      'WHERE CODIGO = :CODPRO'
      'ORDER BY DESCRICAO')
    Left = 464
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'CODPRO'
        ParamType = ptUnknown
      end>
    object QProdCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = 'PRODUTOS_FARDO.CODIGO'
      Required = True
    end
    object QProdCODIGO_BARRAS: TIBStringField
      FieldName = 'CODIGO_BARRAS'
      Origin = 'PRODUTOS_FARDO.CODIGO_BARRAS'
      Size = 13
    end
    object QProdQUANT_VENDA: TIntegerField
      FieldName = 'QUANT_VENDA'
      Origin = 'PRODUTOS_FARDO.QUANT_VENDA'
    end
    object QProdVALOR_VENDA: TIBBCDField
      FieldName = 'VALOR_VENDA'
      Origin = 'PRODUTOS_FARDO.VALOR_VENDA'
      Precision = 18
      Size = 2
    end
    object QProdDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = 'PRODUTOS_FARDO.DESCRICAO'
      Size = 70
    end
    object QProdUNID: TIBStringField
      FieldName = 'UNID'
      Origin = 'PRODUTOS_FARDO.UNID'
      Size = 6
    end
  end
  object DSProd: TDataSource
    DataSet = QProd
    Left = 464
    Top = 192
  end
  object IBTRProd: TIBTransaction
    DefaultDatabase = DM.IBDatabase
    Left = 464
    Top = 240
  end
  object ActionList: TActionList
    OnUpdate = ActionListUpdate
    Left = 360
    Top = 176
    object ActionIncluir: TAction
      Caption = 'Incluir'
      OnExecute = ActionIncluirExecute
      OnUpdate = ActionIncluirUpdate
    end
    object ActionAlterar: TAction
      Caption = 'Alterar'
      OnExecute = ActionAlterarExecute
      OnUpdate = ActionIncluirUpdate
    end
    object ActionExcluir: TAction
      Caption = 'Excluir'
      OnExecute = ActionExcluirExecute
      OnUpdate = ActionIncluirUpdate
    end
    object ActionSalvar: TAction
      Caption = 'Salvar'
      OnExecute = ActionSalvarExecute
      OnUpdate = ActionSalvarUpdate
    end
    object ActionCancelar: TAction
      Caption = 'Cancelar'
      OnExecute = ActionCancelarExecute
      OnUpdate = ActionSalvarUpdate
    end
  end
end
