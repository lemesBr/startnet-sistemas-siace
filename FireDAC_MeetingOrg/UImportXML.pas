...
uses
  ...
  IBCustomDataSet, IBQuery, aDvGlowButton, UBibli1, NxPageControl, RzCmboBx,
  RzDBCmbo, ACBrBase, ACBrDFe, ACBrDFeReport, ACBrDFeDANFeReport;

type
  TImportXML = class(TForm)
    sCoolBar1: TsPanel;
...
uses
  ModulodeDados, UDialog, Func;
{$R *.dfm}

procedure TImportXML.FDvGlowButton4Click(Sender: TObject);
begin
{  try
...
begin
  try
    Nfe.NotasFiscais.LoadFromFile(Arquivo1.Text);
    CarregaDadosNota();
    CarregaForn();
    CarregaTransp;
    CarregaDuplicatas;
    ...
    on E: Exception do
    begin
    //  Application.MessageBox (('Erro ao abrir o XML!' + #13 + E.Message), 'Erro',
     //   mb_applmodal + mb_iconerror + mb_ok);
        MessageDlg(('Erro ao abrir o XML!' + #13 + E.Message),mtInformation, [mbOk],0);
      close;
    end;
  end;
end;
...
begin
  PODE_IMPORTAR := false;
  DadosNfe.show;
  Danfe.FastFile := ExtractFilePath(Application.ExeName) + '\Rel\DANFE.fr3';
...
begin
  ...
  Nota.Open;
  Nota.Insert;
  NotaNUMNF_ENT.AsInteger := Nfe.NotasFiscais.Items[0].Nfe.Ide.nNF;
  NotaSERIE_ENT.AsInteger := Nfe.NotasFiscais.Items[0].Nfe.Ide.cNF;
  NotaDATAEMI_ENT.AsDateTime := Nfe.NotasFiscais.Items[0].Nfe.Ide.dEmi;
  NotaDATAENT_ENT.AsDateTime := date;
  NotaTOTAL_ENT.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Total.ICMSTot.vNF;
  NotaACRES_ENT.AsCurrency := Nfe.NotasFiscais.Items[0]
    .Nfe.Total.ICMSTot.vOutro;
  NotaDESC_ENT.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Total.ICMSTot.vDesc;
  NotaFRETE_ENT.AsCurrency := Nfe.NotasFiscais.Items[0]
    .Nfe.Total.ICMSTot.vFrete;
  NotaICMSFRETE_ENT.AsCurrency := 0;
  if Nfe.NotasFiscais.Items[0].Nfe.Ide.indPag = ipVista then
    NotaTIPOENT.AsString := 'V'
  else
    NotaTIPOENT.AsString := 'P';
  NotaBASE_SUB_TRIB.AsCurrency := Nfe.NotasFiscais.Items[0]
    .Nfe.Total.ICMSTot.vBCST;
  NotaVALOR_ICMS_SUB.AsCurrency := Nfe.NotasFiscais.Items[0]
    .Nfe.Total.ICMSTot.vST;
  NotaTOTAL_PRODUTOS.AsCurrency := Nfe.NotasFiscais.Items[0]
    .Nfe.Total.ICMSTot.vProd;
  NotaCODIFICACAO_FISCAL.AsInteger := Nfe.NotasFiscais.Items[0].Nfe.Ide.modelo;
  NotaCODIGO_ES.AsInteger := Nfe.NotasFiscais.Items[0].Nfe.Ide.cUF;
  NotaCHAVE_NFE.AsString := Nfe.NotasFiscais.Items[0].Nfe.infNFe.ID;
  NotaBCICMS.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Total.ICMSTot.vBC;
  NotaVALOR_ICMS.AsCurrency := Nfe.NotasFiscais.Items[0]
    .Nfe.Total.ICMSTot.vICMS;
  NotaV_PIS.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Total.ICMSTot.vPIS;
  NotaV_COFINS.AsCurrency := Nfe.NotasFiscais.Items[0]
    .Nfe.Total.ICMSTot.vCofins;
  NotaV_IPI.AsCurrency := Nfe.NotasFiscais.Items[0].NFe.Total.ICMSTot.vIPI;
  NotaV_SEG.AsCurrency := Nfe.NotasFiscais.Items[0].NFe.Total.ICMSTot.vSeg;
...
begin
  ...
  FornecedorRAZAO.AsString := Nfe.NotasFiscais.Items[0].Nfe.Emit.xNome;
  FornecedorEND.AsString := Nfe.NotasFiscais.Items[0].Nfe.Emit.EnderEmit.xLgr;
  FornecedorBAIRRO.AsString := Nfe.NotasFiscais.Items[0]
    .Nfe.Emit.EnderEmit.xBairro;
  FornecedorCIDADE.AsString := Nfe.NotasFiscais.Items[0]
    .Nfe.Emit.EnderEmit.xMun;
  FornecedorCEP.AsInteger := Nfe.NotasFiscais.Items[0].Nfe.Emit.EnderEmit.CEP;
  FornecedorUF.AsString := Nfe.NotasFiscais.Items[0].Nfe.Emit.EnderEmit.UF;
  FornecedorCNPJ.AsString := Nfe.NotasFiscais.Items[0].Nfe.Emit.CNPJCPF;
  FornecedorINSC.AsString := Nfe.NotasFiscais.Items[0].Nfe.Emit.IE;
  FornecedorTEL.AsString := Nfe.NotasFiscais.Items[0].Nfe.Emit.EnderEmit.fone;
  FornecedorNUMERO.AsString := Nfe.NotasFiscais.Items[0].Nfe.Emit.EnderEmit.nro;
  FornecedorIBGE.AsInteger := Nfe.NotasFiscais.Items[0].Nfe.Emit.EnderEmit.cMun;
  if Length(FornecedorCNPJ.AsString) > 11 then
    FornecedorFJ.AsString := 'J'
  else
    FornecedorFJ.AsString := 'F';
  Fornecedor.Post;
  MesmoEstado := dm.MovimentoMesmoUF( Nfe.NotasFiscais.Items[0].Nfe.Emit.EnderEmit.UF );
...
begin
  ...
  Transp.Open;
  Transp.Insert;
  TranspNOME.AsString := Nfe.NotasFiscais.Items[0].Nfe.Transp.Transporta.xNome;
  TranspEND.AsString := Nfe.NotasFiscais.Items[0].Nfe.Transp.Transporta.xEnder;
  TranspCIDADE.AsString := Nfe.NotasFiscais.Items[0].Nfe.Transp.Transporta.xMun;
  TranspUF.AsString := Nfe.NotasFiscais.Items[0].Nfe.Transp.Transporta.UF;
  TranspCNPJ.AsString := Nfe.NotasFiscais.Items[0]
    .Nfe.Transp.Transporta.CNPJCPF;
  TranspINSC.AsString := Nfe.NotasFiscais.Items[0].Nfe.Transp.Transporta.IE;
  TranspPLACA.AsString := Nfe.NotasFiscais.Items[0].Nfe.Transp.veicTransp.placa;
  TranspUF_PLACA.AsString := Nfe.NotasFiscais.Items[0].Nfe.Transp.veicTransp.UF;
...
begin
  ...
  if true then
    ...
    begin
      Dup.Insert;
      DupDOC.AsString := Nfe.NotasFiscais.Items[0].Nfe.Cobr.Fat.nFat;
      DupVenc.AsDateTime := Nfe.NotasFiscais.Items[0].Nfe.Ide.dEmi;
      DupValor.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Cobr.Fat.vLiq;
      Dup.Post;
    end
    else
    begin
      ...
      begin
        Dup.Insert;
        DupDOC.AsString := Nfe.NotasFiscais.Items[0].Nfe.Cobr.Dup.Items[I].nDup;
        DupVenc.AsDateTime := Nfe.NotasFiscais.Items[0].Nfe.Cobr.Dup.Items[I]
          .dVenc;
        DupValor.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Cobr.Dup.Items[I]
          .vDup;
        Dup.Post;
      end;
    end;
...
begin
  ...
  Items.EmptyDataSet;
  Items.Open;
  Prod := Nfe.NotasFiscais.Items[0].Nfe.Det.Count;
  for I := 0 to Prod - 1 do
  begin
    Items.Insert;
    ItemsCOD_PRO.AsString := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Prod.cProd;
    ItemsEAN.AsString := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Prod.cEAN;
    ItemsPRODUTO.AsString := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Prod.xProd;
    ItemsNCM.AsString := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Prod.NCM;
    ItemsMEDIDA.AsString := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Prod.uTrib;
    ItemsQUANT.AsFloat := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Prod.qTrib;
    ItemsVALOR_UNID.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Prod.vUnTrib;
    ItemsVALOR_TOTAL.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Prod.vProd;
    ItemsORIG.AsString := OrigToStr
      (Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Imposto.ICMS.orig);
    if Nfe.NotasFiscais.Items[0].Nfe.Emit.CRT = crtSimplesNacional then
      ItemsCST.AsString := CSOSNICMSToStr
        (Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Imposto.ICMS.CSOSN)
    else
      ItemsCST.AsString := CSTICMSToStr
        (Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Imposto.ICMS.CST);
    ItemsBC.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.ICMS.vBC;
    ItemsALIQ_ICMS.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.ICMS.pICMS;
    ItemsVALOR_ICMS.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.ICMS.vICMS;
    ItemsBCST.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.ICMS.vBCST;
    ItemsALIQ_ST.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.ICMS.pICMSST;
    ItemsVALOR_ST.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.ICMS.vICMSST;
    ItemsALIQ_IPI.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.IPI.pIPI;
    ItemsVALOR_IPI.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.IPI.vIPI;
    ItemsBC_IPI.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.IPI.vBC;
    ItemsDESC.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Prod.vDesc;

    sIPI := trim(CSTIPIToSTR(Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
          .Imposto.IPI.CST));
    if sIPI = '99' then
    ...
    ItemsPRedBC.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.ICMS.pRedBC;
    ItemsPRedBCST.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.ICMS.pRedBCST;

    ItemsPIS_CST.AsString := CSTPISToStr
      (Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Imposto.PIS.CST);
    ItemsPIS_BC.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.PIS.vBC;
    ItemsPIS_ALIQ.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.PIS.pPIS;
    ItemsPIS_VALOR.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.PIS.vPIS;

    ItemsCOFINS_CST.AsString := CSTCOFINSToStr
      (Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Imposto.COFINS.CST);
    ItemsCOFINS_BC.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.COFINS.vBC;
    ItemsCOFINS_ALIQ.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.COFINS.pCOFINS;
    ItemsCOFINS_VALOR.AsCurrency := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I]
      .Imposto.COFINS.vCofins;
    ItemsCFOP.AsString := Nfe.NotasFiscais.Items[0].Nfe.Det.Items[I].Prod.CFOP;
...
begin
  ...
  if dm.IBTransaction.InTransaction then
    dm.IBTransaction.Commit;
  dm.IBTransaction.StartTransaction;
  dm.QConsulta.sql.Text := 'select * from FORNECEDORES where CNPJ=:CNPJ';
  ...
  begin
    ...
    COD_FOR := dm.QConsulta.FieldByName('CODIGO').AsInteger;
...
begin
  ...
  begin
    ...
    dm.QConsulta.sql.Text :=
      'select * from PRODUTOS where CODIGO_BARRAS=:BARRAS';
    ...
    begin
      Items.Edit;
      ItemsCOD_LOCAL.AsInteger := dm.QConsulta.FieldByName('CODIGO').AsInteger;
...
begin
  ...
  try
    ...
    dm.QConsulta.close;
    dm.QConsulta.sql.Text := 'Select MAX(COD_FOR) ULTIMO from fornecedores';
    dm.QConsulta.Open;
    ...
    dm.QConsulta.close;
    dm.QConsulta.sql.Text :=
      'insert into fornecedor( CODIGO, TP, NOME, ENDERECO, BAIRRO, CIDADE, CEP, ESTADO, CNPJ, IE, TELEFONE, NUMERO, "codigo_ibge"'+
    ...
    begin
     { Application.MessageBox
        (PAnsiChar('Erro ao Cadastrar fornecedor!' + #13 + E.Message), 'Erro',
        mb_applmodal + mb_iconerror + mb_ok); }
       MessageDlg(('Erro ao Cadastrar fornecedor!' + #13 + E.Message), mtInformation,[mbOk], 0);
      COD_FOR := 0;
      dm.IBTransaction.Rollback;
...
begin
  ...
  {  dm.QConsulta.sql.Text :=
      ...
      ' SAIR_REL_RESUMO_ALIQ, TIPO, REGISTRADO_ECF, CALCULA_PIS_COFINS, USA_CSOSN)' + ' VALUES (:CFOP, :DESC, :DESTACA_ICMS, :SAIR_REL, :TIPO, :ECF, :PIS, :CSOSN);';
       ...
       dm.QConsulta.sql.Text :=
      'INSERT INTO CFOP (CODIGO_CFOP, DESCRICAO) VALUES (:CFOP, :DESC)';
   ...
   { if NotaBCICMS.AsFloat > 0 then
    ...
    begin
     { Application.MessageBox
        (PAnsiChar('Erro ao Cadastrar CFOP!' + #13 + E.Message), 'Erro',
        mb_applmodal + mb_iconerror + mb_ok);   }
        MessageDlg(('Erro ao Cadastrar CFOP!' + #13 + E.Message), mtInformation,[mbOk], 0);
      COD_CFOP := 0;
      dm.IBTransaction.Rollback;
    end;
  end;
...
begin
 { Items.Locate('SEQ', SEQ, []);
  ...
  try
    ...
    begin
      Application.MessageBox
        (PAnsiChar('Erro ao Cadastrar Produto!' + #13 + E.Message), 'Erro',
        mb_applmodal + mb_iconerror + mb_ok);
      dm.IBTransaction.Rollback;
...
begin
  ...
  begin
     if not VerificaTributacoes then
        begin
           KDialog( 'Precisa Verificar todas tributações de entrada', 'Importação de XML', 'INFO');
           Exit;
        end;
     ImportarTudo;
  end;
...
begin
 { try
    ...
    dm.QConsulta.sql.Text :=
      'select max( CODIGO_NOTA ) from NOTASFISCAIS';
    ...
    dm.QConsulta.sql.Clear;
    dm.QConsulta.sql.Text := 'select * from NOTASFISCAIS where CHAVE_NFE=:CHAVE';
    dm.QConsulta.ParamByName('CHAVE').AsString := NotaCHAVE_NFE.AsString;
    dm.QConsulta.Open;
    if not dm.QConsulta.IsEmpty then
    begin
      dm.QConsulta.close;
      ShowMessage('Nota ja foi importada antes');
      result := 0;
      exit;
    end;
    dm.QConsulta.close;
      ...
      begin
        ...
        if DMI.PRODUTO_T_E.IsEmpty then
           begin
              ...
              'Importação de Nota Fiscal', 'ERRO' );
              DMI.IBTRImport.Rollback;
              close;
              Exit;
...
{        dmi.ItensNotaBC_ST.AsCurrency := ImportXML.ItemsBCST.AsCurrency;
     ...
     {   dmi.ItensNota.Post;
        dmi.ItensNota.close;
            ...
            on E: Exception do
               begin
                  KDialog( 'Erro no Item ' + ItemsSEQ.AsString + #13 + E.Message, 'Erro ao importar Nota', 'ERRO' );
                        dm.IBTransaction.Rollback;
                  dmi.IBTRImport.Rollback;
                  result := false;
               end;
...
var
  ...
  begin
   { ShowMessage('Nota fiscal não foi salva ouve problemas na sequencia!');
    if dm.IBTransaction.InTransaction then
      dm.IBTransaction.Rollback;
    if dmi.IBTRImport.InTransaction then
      dmi.IBTRImport.Rollback; }
  end;
