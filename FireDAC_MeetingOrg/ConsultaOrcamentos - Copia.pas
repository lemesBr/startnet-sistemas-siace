...
begin
  if not odd(DM.Sds_orcamento.RecNo) then
...
procedure TFormConsultaOrcamentos.suiDBGrid2DrawColumnCell(Sender: TObject;
  ...
  State: TGridDrawState);
begin
  if not odd(DM.Sds_orcamento_itens.RecNo) then
  if not (gdselected in State) then
    begin
...
//formVendas.Sds_Orcamentos.Text             := DM.sds_orcamentoSTATUS.Text;
//formVendas.Sds_OrcamentosTP.text                 := DM.sds_orcamentoTP.Text;
