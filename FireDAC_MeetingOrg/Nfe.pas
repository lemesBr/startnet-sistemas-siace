...
uses IniFiles, ShellAPI,  ACBrDFeUtil, pcnConversao, pcnConversaoNFe, pcnNFeRTXT,
     ...
     Mylabel, Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
...
uses Ubibli1, FileCtrl, pcnNFe, ufrmStatus, ACBrNFeNotasFiscais, DateUtils, 
  ModulodeDados,  Principal, gerNFE, pcnProcNFe, gerNFce,
  FrmEnviaParaEmail, ACBrNFeConfiguracoes, ACBrNFeWebServices, IBQuery;
...
begin
  ...
  BEGIN
    ...
    begin
      ...
      MemoDados.Lines.Add('Nf-e ja enviada para sefaz, AGUARDANDO VALIDAÇÃO...');
      close;
    end;
    ...
    begin
      MemoDados.Lines.Add('');
      MemoDados.Lines.Add('Envio NFce');
      MemoDados.Lines.Add('Nf-e ja enviada para sefaz, base de dados será Atualizada...');
      DM.SDS_NFE.Edit;
      DM.SDS_NFENR_LOTE_NFE.Text := (vNumLote);
  ...
  begin
   ...
   // DM.ACBrNFe1.NotasFiscais.Imprimir;
    ...
    if Application.MESSAGEBOX('Confirma o envio do email ao destinatário da NF-e?', 'Confirmar', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON1) = ID_YES then
    begin
      ...
      sCaminhoXML:= DM.ACBrNFe1.Configuracoes.Arquivos.PathNFe + '\' +
        sCNPJEmpresa + '\NFe\'+ tempdata + '\' + Trim(dm.SDS_NFECHAVE_ACESSO_NFE.AsString) + '-nfe.xml';
  ...
  with DM.ACBrNFe1.NotasFiscais.Add.NFe do
   begin
    ...
    // Ide.indPag    := ipVista;
     ...
     Ide.verProc   := 'SIACE 1.0.0.70'; //Versão do seu sistema
     ...
     Ide.finNFe    := fnDevolucao;
      ...
      // se a nota e interestadual ou não
       ...
       begin
        ...
        begin
         with FormGefaNFe.QBuscaItens1 do
            begin
               close;
               Parambyname('cod_nota').AsInteger := strtoint(dm.sds_nfe_itensCD_ATUALIZACAO.Text);
               Parambyname('codpro').AsInteger   := strtoint(DM.sds_nfe_itensCODIGO_PROD.text);
               open;
               Last;
            end;
         ...
         //VENDAS
           ...
           begin
             ...
             begin
               ...
               tributo := ExatoCurrency((dm.sds_nfe_itensVL_TOTAL.ASFLOAT * FormGefaNFe.QBuscaItensALIC_REVENDA.AsFloat / 100 + BaseICMS_T),2);
               tributos := 'Valor Aproximado dos Tributos : R$ '+ FloatToStr(tributo) + ' PERC ' + FloatToStr(FormGefaNFe.QBuscaItensALIC_REVENDA.AsFloat) +' Fonte: IBPT';
             end
             else
             begin
           ...
           begin
             ...
             begin
               BaseICMS_T:=  ((ConvertCurrency(FormGefaNFe.edtPercentualICMSST.text) / 100) * (ConvertCurrency(FormGefaNFe.edtBaseICMSST.Text) / 100)) * dm.sds_nfe_itensVL_TOTAL.ASFLOAT;
               tributo := ExatoCurrency((dm.sds_nfe_itensVL_TOTAL.ASFLOAT * FormGefaNFe.QBuscaItens1ALIC_REVENDA.AsFloat / 100+ BaseICMS_T),2);
               tributos := 'Valor Aproximado dos Tributos : R$ '+ FloatToStr(tributo) + ' PERC ' + FloatToStr(FormGefaNFe.QBuscaItens1ALIC_REVENDA.AsFloat) +' Fonte: IBPT';
             end
             else
             begin
...
//         infAdProd   := FDic +#13+#10+ tributos;
         ...
         //Declaração de Importação. Pode ser FDicionada várias através do comando Prod.DI.Add
{         with Prod.DI.Add do
          begin
            nDi         := '';
            dDi         := now;
...
//            end;
             ...
             begin
               ...
               begin
                 BaseICMS_T:=  ((ConvertCurrency(FormGefaNFe.edtPercentualICMSST.text) / 100) * (ConvertCurrency(FormGefaNFe.edtBaseICMSST.Text) / 100)) * dm.sds_nfe_itensVL_TOTAL.ASFLOAT;
                 tributo := ExatoCurrency((dm.sds_nfe_itensVL_TOTAL.ASFLOAT * FormGefaNFe.QBuscaItensALIC_REVENDA.AsFloat / 100 + BaseICMS_T),2);
                 tributo1 := ExatoCurrency((dm.sds_nfe_itensVL_TOTAL.ASFLOAT * FormGefaNFe.QBuscaItensALIC_REVENDA.AsFloat / 100 + BaseICMS_T),2);
               end
               else
               begin
                 tributo := ExatoCurrency((dm.sds_nfe_itensVL_TOTAL.ASFLOAT * FormGefaNFe.QBuscaItensALIC_NAC.AsFloat / 100),2);
             ...
             begin
               ...
               begin
                 BaseICMS_T:=  ((ConvertCurrency(FormGefaNFe.edtPercentualICMSST.text) / 100) * (ConvertCurrency(FormGefaNFe.edtBaseICMSST.Text) / 100)) * dm.sds_nfe_itensVL_TOTAL.ASFLOAT;
                 tributo := ExatoCurrency((dm.sds_nfe_itensVL_TOTAL.ASFLOAT * FormGefaNFe.QBuscaItens1ALIC_REVENDA.AsFloat / 100 + BaseICMS_T),2);
                 tributo1 := ExatoCurrency((dm.sds_nfe_itensVL_TOTAL.ASFLOAT * FormGefaNFe.QBuscaItens1ALIC_REVENDA.AsFloat / 100 + BaseICMS_T),2);
               end
               else
               begin
                  tributo := ExatoCurrency((dm.sds_nfe_itensVL_TOTAL.ASFLOAT * FormGefaNFe.QBuscaItens1ALIC_NAC.AsFloat / 100),2);
    ...
    IF FormGefaNFe.Edit11.text >'' then
      ...
      with Transp.Vol.Add do
       begin
         ...
         //Lacres do volume. Pode ser FDicionado vários
         //Lacres.Add.nLacre := '';
       end;
  ...
  //  ShowMessage('TESTE 2');
    ...
    if DM.SDS_NFEVL_PRAZO.AsFloat > 0 then
    IF  NOT DM.sds_crediario.Eof THEN
    BEGIN
    parcela := strtoint(Copy(DM.SDS_CrediarioPARCELA.Text , Pos('/', DM.SDS_CrediarioPARCELA.Text)+ 1, Length(DM.SDS_CrediarioPARCELA.Text)));
    begin
    For E:=1 to Contador do
     begin
      with Cobr.Dup.Add do
       begin
        ...
        // vDup  := StrToFloatDef(DM.SDS_crediarioValor_compra.text,0.00);
         vDup  := (DM.SDS_NFEVL_TOTAL.ascurrency - DM.SDS_NFEVL_AVISTA.AsFloat) / parcela;
       end;
      ...
      BEGIN
       ...
       begin
         nDup  := '1/1';
         dVenc := now+30;
         vDup  := (DM.SDS_NFEVL_TOTAL.ascurrency - DM.SDS_NFEVL_AVISTA.AsFloat);
       end;

      end;

   //    ShowMessage('TESTE 3');
      // showmessage('Teste 1 ok ');
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        MemoDados.Lines.Add('NFC-e nº '+ DM.SDS_NFCENR_NOTA.Text + 'ja enviada para sefaz, base de dados sera atualizada...');
  ...
  begin
    ...
    begin
    {  if CheckBox2.Checked then
        DM.ACBrNFeDANFEFR1.desc := 'Emitida em contingência'
      else
...
begin
  ...
  with DM.ACBrNFe1.NotasFiscais.Add.NFe do
   begin
     ...
     Ide.verProc   := 'SIACE 1.0.0.61'; //Versão do seu sistema
...
//         infAdProd      := 'Informação FDicional do Produto';
  //  ShowMessage('teste1');
