...
uses
  ...
  aDvGlowButton,  DBGrids, aDvPanel, FireDAC.Stan.Intf,
...
begin
  ...
  begin // bloqueado
       ...
       // if application.MessageBox(texto,'Atenção',mb_yesno+MB_ICONWARNING) = idyes then
        if MessageDlg('Cliente com crédito Bloqueado!, Deseja continuar?', mtConfirmation,[mbyes,mbno], 0) = idyes  then
          begin
...
//        if application.MessageBox(texto,'Atenção',mb_yesno+MB_ICONWARNING) = idyes then
         if MessageDlg('Cliente INATIVO!, Deseja continuar?', mtConfirmation,[mbyes,mbno], 0) = idyes  then
    ...
    BEGIN
     ...
     begin
       ...
       //  if application.MessageBox(,'Atenção',mb_yesno+MB_ICONWARNING) = idyes then
         if MessageDlg('Este cliente possue um limite disponível de:'+#13+' ---> R$ '+formatfloat('###,###,##0.00',(qrcliente.FIELDBYNAME('LIMITE_DE_CREDITO').ASFLOAT - vlimite))+'  <--- '+#13+'É necessário autorização para prosseguir! '+#13+'Deseja continuar?', mtConfirmation,[mbyes,mbno], 0) = idyes  then
        begin
          // if not frmPrincipal.autentica('Liberar venda',3) then
           if FormPrincipal.Label3.Caption ='N'then
           begin
