...
uses
  ...
  sBitBtn, AdvUtil, AdvObj, AdvGrid, DBAdvGrid;
...
begin
       ...
       DM.DTS_PRODUTOS.DataSet.Filtered := False;
       DM.DTS_PRODUTOS.DataSet.Filter:= 'codigo_barras LIKE ' +QuotedStr('%'+'-1'+'%');
       DM.DTS_PRODUTOS.DataSet.Filtered:= True;
end else
begin
 DM.SDS_PRODUTOS.DataSet.Filtered := False;
end;
end;
...
begin
  ...
  begin
   ...
   BEGIN
      ...
      begin
      DM.Sds_produtos.Active := false;
      DM.Sds_produtos.DataSet.CommandText:= 'select * from PRODUTOS where CODORIGINAL like ' + #39 + '%' + Edit1.Text + '%' + #39+'ORDER BY DESCRICAO';
      DM.Sds_produtos.Active := True;
      ...
      begin
      DM.Sds_produtos.Active := false;
      DM.Sds_produtos.DataSet.CommandText:= 'select * from PRODUTOS where COD_PACOTE like ' + #39 + '%' + Edit1.Text + '%' + #39+'ORDER BY DESCRICAO';
      DM.Sds_produtos.Active := True;
       ...
       Label1.Caption := 'N° Produtos Ativos Fitrados: ' + IntToStr(DM.Sds_produtos.RecordCount);
        ...
        IF Edit1.Text = '' then
         ...
         Label1.Caption := 'N° Produtos Ativos Fitrados: ' + IntToStr(DM.Sds_produtos.RecordCount);
            ...
            If not DM.Sds_produtos.locate('CODIGO', Edit1.Text,[])=True then
            begin
             SHOWMESSAGE('Produto Não Localizado');
             Edit1.SetFocus;
            end;
