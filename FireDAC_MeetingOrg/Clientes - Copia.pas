...
uses
  ...
  RzPanel, RzTabs, FDvReflectionImage, FDvGlowButton, wwdbedit, Wwdotdot,
  Wwdbcomb, CurrEdit, RzEdit, RzDBEdit, RzDBBnEd, AlignEdit, PageView,
  Collection, Wwdbigrd, Wwdbgrid, TFlatPanelUnit, IBCustomDataSet, IBQuery,
...
uses ModulodeDados, Ubibli1, Z_RotinasGerais, Principal, Acesso, ConsClientes,
  ...
  ClientesContato, ClientesDocumento;
...
if db_nome.Text > '' then
 ...
 if DM.SDS_CLIentes.Locate('NOME_RS',db_nome.Text, []) = True then
 BEGIN
...
Try
   inserindo := True;
   //MsgConfirmacao.HTMLText.text:= 'Informe o tipo de cliente (Pessoa)!!!';
  // MsgConfirmacao.Button1Caption:= 'Física';
  // MsgConfirmacao.Button2Caption:= 'Jurídica';
  // if Application.MESSAGEBOX('Informe o tipo da Pessoa (Sim) Fisica (Não) Júridica?', 'Confirmar', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON1) = ID_NO then

    If MessageDlgAde('Informe o tipo da Pessoa (Física ou Jurídica).',MtInformation,[MbYes, MbNo],0, mrNo, True)=Mrno Then

  // IF MsgConfirmacao.ClickButton(1) then
   begin
...
begin
 Try
      begin
          ...
          IF wwDBComboBox1.ItemIndex = 2 THEN BEGIN DM.SDS_CLI.FIELDBYNAME('ATIVO').AsString := 'O'; END;
          IF wwDBComboBox1.ItemIndex = 1 THEN BEGIN DM.SDS_CLI.FIELDBYNAME('ATIVO').AsString := 'B';END;
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage('Registro deletetado com sucesso, + '+#13+
      'para concluir a operação é necessário salvar o cliente!');
    end;
  end
  else
...
begin
  ...
  begin
    ...
    if dsDocumento.DataSet.FieldByName('POSSUE').AsString = 'S' then
      img25.Draw(gridDocumento.Canvas,Rect.Left +3,Rect.Top + 1,0)
    else
      img25.Draw(gridDocumento.Canvas,Rect.Left+3,Rect.Top + 1,0);
  end
  else
  ...
  begin
    gridDocumento.Canvas.FillRect(Rect);
    img25.Draw(gridDocumento.Canvas,Rect.Left +3,Rect.Top + 1,1)
  end
  else
  if Column.FieldName = 'DEL' then
  begin
    gridDocumento.Canvas.FillRect(Rect);
    img25.Draw(gridDocumento.Canvas,Rect.Left +3,Rect.Top + 1,2)
  end;
end;
...
begin
  ...
  begin
    if dsDocumento.DataSet.RecordCount > 0 then
    begin
      if MessageDlg('Deseja realmente excluir esse documento?' + #13 +
              'após excluido não podera ser desfeito.', mtInformation, [mbYes],1) = mrYes then
      begin
