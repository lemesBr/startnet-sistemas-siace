...
begin
  if TipoRelatorio = 'V' then
     FormEcf.ACBrECF1.CupomVinculado( edCupom.Text, edFPG.Text, edCNF.Text,
        StrToFloatDef( edValor.Text,0 ), mRelat.Lines,
        StrToIntDef(edVias.Text,1) )
  else
     FormEcf.ACBrECF1.RelatorioGerencial(mRelat.Lines,StrToIntDef(edVias.Text,1), StrToIntDef(edCupom.Text, 0));
end;

procedure TfrRelatorio.FormShow(Sender: TObject);
begin
//  mRelat.WrapAtValue := FormEcf.ACBrECF1.Colunas ;
...
//  edCupom.Visible := edFPG.Visible ;
  ...
  if edCupom.Visible then
     edCupom.Text := FormEcf.ACBrECF1.NumCupom ;
     
  edCNF.Text      := '' ;
  edCNF.Visible   := edFPG.Visible and
                    (FormEcf.ACBrECF1.Modelo in [ecfDaruma, ecfSchalter]) ;
...
begin
  FormEcf.FormasdePagamento1Click( Sender );
end;
...
begin
  FormEcf.CarregaComprovantesNAOFiscais1Click( Sender );
end;
