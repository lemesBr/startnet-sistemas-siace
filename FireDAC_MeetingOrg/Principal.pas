...
uses
  ...
  Dialogs, ExtCtrls, Menus, ComCtrls, Buttons,IniFiles,
  acAlphaImageList, ACBrBase, ACBrCalculadora, ExtDlgs,
  sDialogs, AppEvnts, StdCtrls, aDvGlowButton, sLabel, advToolBar,
  WinSkinData, advOfficePager, advOfficePagerStylers, advPanel,
  advSmoothMessageDialog, RDprint, DB, IBCustomDataSet, IBQuery, ActnList,
  ImgList, Registry, RLConsts, Mask, rxToolEdit, IdStackWindows, acPNG, ShellAPI,
...
uses Z_RotinasGerais, clientes, Acesso, ModulodeDados, Unidades,
  ...
  ConfBancos, TitulosLista, boletos,  BoletosBanc, Contrato,
  ...
  contasreceber_ficha_varios, U_Principal, Urelgrupotrib,
  ...
  frmOrigemFlorestal, frmPatios, Biblioteca;
...
begin
  ...
  try
    X := TIniFile.Create(ExtractFilePath(Application.ExeName)+'WCI.ini');
...
begin
  ...
  begin
    if FileExists('C:\SIACE\BACKUP.EXE') and
      (UpperCase(Trim(TBiblioteca.LerIni('siace.ini', 'DADOS', 'DATABASE'))) = UpperCase('C:\SIACE\SIACE.GDB'))
    then
    begin
      ...
      then
        ShellExecute(HWND_DESKTOP,'OPEN', PChar('C:\SIACE\BACKUP.EXE'), nil, nil, SW_SHOWNORMAL);
    end;
  end;
end;
...
begin
  suiStatusBar1.Panels[0].Text := 'Empresa: ' + dm.SDS_EmpresaNOME_FANTASIA.Text;
  suiStatusBar1.Panels[1].Text := 'Telefone: ' + dm.SDS_EmpresaTELEFONE.Text;
...
begin
   { FormDonwloadNfe:=TFormDonwloadNfe.Create(self);
    ...
    Application.CreateForm( TF_downloadnfe, F_downloadnfe );
    F_downloadnfe.Showmodal;
end;
