...
begin
DM.Sds_cidades.Active := FALSE;
DM.Sds_cidades.DataSet.CommandText:= 'SELECT * FROM CIDADES order by NOME asc';
DM.Sds_cidades.Active := TRUE;
...
begin
DM.Sds_cidades.Active := FALSE;
DM.Sds_cidades.DataSet.CommandText:= 'SELECT * FROM CIDADES WHERE NOME LIKE ' + #39 + '%' + Edit1.Text + '%' + #39+'order by NOME asc';
DM.Sds_cidades.Active := TRUE;
end;
