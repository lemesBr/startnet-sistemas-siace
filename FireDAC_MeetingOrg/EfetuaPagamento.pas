...
begin
  if TipoCupom <> 'N' then
   begin
     FormEcf.ACBrECF1.EfetuaPagamento( edCod.Text, StrToFloat( edValor.Text),
                                     edObs.Text ,cbVinc.Checked );
     FormEcf.mResp.Lines.Add( 'Efetua Pagamento: '+edCod.Text +
                            ' Valor: '+edValor.Text +
                            ' Obs: '+edObs.Text +
   ...
   begin
     FormEcf.ACBrECF1.EfetuaPagamentoNaoFiscal( edCod.Text, StrToFloat( edValor.Text),
                                     edObs.Text ,cbVinc.Checked );
     FormEcf.mResp.Lines.Add( 'Efetua Pagamento Não Fiscal: '+edCod.Text +
                            ' Valor: '+edValor.Text +
                            ' Obs: '+edObs.Text +
                            ' Vinc: '+IfThen(cbVinc.Checked,'S','N') );
   end ;
...
begin
  lTotalAPAGAR.Caption := FloatToStr( FormEcf.ACBrECF1.Subtotal ) ;
  lTotalPago.Caption   := FloatToStr( FormEcf.ACBrECF1.TotalPago ) ;
end;
...
begin
   FormEcf.ACBrECF1.EstornaPagamento(EdtTipoCanc.Text, EdtTipoNovo.Text,
