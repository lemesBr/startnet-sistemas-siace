object FormPedidoCompra: TFormPedidoCompra
  Left = 488
  Top = 269
  Caption = 'Emiss'#227'o de Pedido de Compras'
  ClientHeight = 253
  ClientWidth = 549
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 549
    Height = 41
    Align = alTop
    TabOrder = 0
    object myLabel3d1: TmyLabel3d
      Left = 91
      Top = 5
      Width = 358
      Height = 25
      Caption = 'Rela'#231#227'o de Produtos Para Compra'
      Color = clWindow
      Font.Charset = ANSI_CHARSET
      Font.Color = clGray
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Transparent = True
      AStyle3D = Resit3d
      AShadeLTSet = False
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 212
    Width = 549
    Height = 41
    Align = alBottom
    TabOrder = 1
    object suiButton7: TsuiButton
      Left = 228
      Top = 6
      Width = 97
      Height = 27
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Caption = 'Imprimir'
      AutoSize = False
      ParentFont = False
      UIStyle = FromThemeFile
      TabOrder = 0
      Transparent = False
      ModalResult = 0
      FocusedRectMargin = 2
      Glyph.Data = {
        
        F8F8F8F8F8F8F8F8F8F8F8F8F8F8F8F80000}
      Layout = blGlyphLeft
      Spacing = 4
      MouseContinuouslyDownInterval = 100
      OnClick = suiButton7Click
      ResHandle = 0
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 41
    Width = 549
    Height = 171
    Align = alClient
    TabOrder = 2
    object Label5: TLabel
      Left = 4
      Top = 60
      Width = 71
      Height = 13
      Caption = 'Data Pedido:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object RxLabel1: TRxLabel
      Left = 4
      Top = 27
      Width = 69
      Height = 13
      Caption = 'Fornecedor:'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label1: TLabel
      Left = 196
      Top = 60
      Width = 7
      Height = 13
      Caption = 'a'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label2: TLabel
      Left = 4
      Top = 98
      Width = 37
      Height = 13
      Caption = 'Grupo:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 4
      Top = 132
      Width = 58
      Height = 13
      Caption = 'Categoria:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DateEdit3: TDateEdit
      Left = 79
      Top = 57
      Width = 92
      Height = 19
      Ctl3D = False
      NumGlyphs = 2
      ParentCtl3D = False
      TabOrder = 1
    end
    object DateEdit4: TDateEdit
      Left = 223
      Top = 57
      Width = 93
      Height = 19
      Ctl3D = False
      NumGlyphs = 2
      ParentCtl3D = False
      TabOrder = 2
    end
    object Panel7: TPanel
      Left = 175
      Top = 25
      Width = 279
      Height = 23
      BevelInner = bvRaised
      BevelOuter = bvLowered
      Color = 15263976
      TabOrder = 6
      object RxLabel6: TRxLabel
        Left = 4
        Top = 3
        Width = 269
        Height = 16
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clActiveCaption
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = True
      end
    end
    object fornec: TComboEdit
      Left = 79
      Top = 25
      Width = 92
      Height = 24
      CharCase = ecUpperCase
      ClickKey = 113
      Ctl3D = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      GlyphKind = gkEllipsis
      NumGlyphs = 1
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      Text = ''
      OnButtonClick = fornecButtonClick
    end
    object grupo: TComboEdit
      Left = 79
      Top = 97
      Width = 92
      Height = 24
      CharCase = ecUpperCase
      ClickKey = 113
      Ctl3D = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      GlyphKind = gkEllipsis
      NumGlyphs = 1
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 4
      Text = ''
      OnButtonClick = grupoButtonClick
    end
    object Panel4: TPanel
      Left = 174
      Top = 97
      Width = 279
      Height = 23
      BevelInner = bvRaised
      BevelOuter = bvLowered
      Color = 15263976
      TabOrder = 7
      object grupo2: TRxLabel
        Left = 4
        Top = 3
        Width = 269
        Height = 16
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clActiveCaption
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = True
      end
    end
    object categoria: TComboEdit
      Left = 79
      Top = 129
      Width = 92
      Height = 24
      CharCase = ecUpperCase
      ClickKey = 113
      Ctl3D = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      GlyphKind = gkEllipsis
      NumGlyphs = 1
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 5
      Text = ''
      OnButtonClick = categoriaButtonClick
    end
    object Panel5: TPanel
      Left = 175
      Top = 129
      Width = 279
      Height = 23
      BevelInner = bvRaised
      BevelOuter = bvLowered
      Color = 15263976
      TabOrder = 8
      object categoria2: TRxLabel
        Left = 4
        Top = 3
        Width = 269
        Height = 16
        AutoSize = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clActiveCaption
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = True
      end
    end
    object CheckBox1: TCheckBox
      Left = 344
      Top = 56
      Width = 97
      Height = 17
      Caption = 'Geral'
      TabOrder = 3
    end
  end
  object qry_pedidos: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select * from V_PRODUTOS_P_COMPRA')
    SQLConnection = DM.SQLC
    Left = 296
    Top = 32
    object qry_pedidosCOD_EMPRESA: TIntegerField
      FieldName = 'COD_EMPRESA'
    end
    object qry_pedidosCODIGO_SEQ: TIntegerField
      FieldName = 'CODIGO_SEQ'
      Required = True
    end
    object qry_pedidosCOD_PRODUTO: TIntegerField
      FieldName = 'COD_PRODUTO'
    end
    object qry_pedidosCOD_USUARIO: TIntegerField
      FieldName = 'COD_USUARIO'
    end
    object qry_pedidosDATA_FALTA: TDateField
      FieldName = 'DATA_FALTA'
    end
    object qry_pedidosFRACAO: TIntegerField
      FieldName = 'FRACAO'
    end
    object qry_pedidosQUANT_FRACAO: TIntegerField
      FieldName = 'QUANT_FRACAO'
    end
    object qry_pedidosQUANT_INTEIRA: TIntegerField
      FieldName = 'QUANT_INTEIRA'
    end
    object qry_pedidosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object qry_pedidosPRECO_CUSTO: TFMTBCDField
      FieldName = 'PRECO_CUSTO'
      Precision = 15
      Size = 2
    end
    object qry_pedidosPRECO_VENDA: TFMTBCDField
      FieldName = 'PRECO_VENDA'
      Precision = 15
      Size = 2
    end
    object qry_pedidosPRECO_VENDA2: TFMTBCDField
      FieldName = 'PRECO_VENDA2'
      Precision = 15
      Size = 2
    end
    object qry_pedidosMED_GENERICO: TStringField
      FieldName = 'MED_GENERICO'
      Size = 3
    end
    object qry_pedidosQTD_FRACIONADA: TFMTBCDField
      FieldName = 'QTD_FRACIONADA'
      Precision = 15
      Size = 2
    end
    object qry_pedidosQTD_CAIXA: TFMTBCDField
      FieldName = 'QTD_CAIXA'
      Precision = 15
      Size = 0
    end
    object qry_pedidosMED_CONTROLADO: TStringField
      FieldName = 'MED_CONTROLADO'
      Size = 1
    end
    object qry_pedidosPRINCIPIO_ATIVO: TStringField
      FieldName = 'PRINCIPIO_ATIVO'
      Size = 50
    end
    object qry_pedidosLABORATORIO: TStringField
      FieldName = 'LABORATORIO'
      Size = 50
    end
    object qry_pedidosCOD_LABORATORIO: TIntegerField
      FieldName = 'COD_LABORATORIO'
    end
    object qry_pedidosCODIGO_MARCA: TIntegerField
      FieldName = 'CODIGO_MARCA'
    end
    object qry_pedidosNOME_GRUPO: TStringField
      FieldName = 'NOME_GRUPO'
      Size = 40
    end
    object qry_pedidosNOME_SUBGRUPO: TStringField
      FieldName = 'NOME_SUBGRUPO'
      Size = 40
    end
    object qry_pedidosNOME_MARCA: TStringField
      FieldName = 'NOME_MARCA'
      Size = 40
    end
    object qry_pedidosCODIGO_SUBGRUPO: TIntegerField
      FieldName = 'CODIGO_SUBGRUPO'
    end
    object qry_pedidosCODIGO_GRUPO: TIntegerField
      FieldName = 'CODIGO_GRUPO'
    end
    object qry_pedidosCATEGORIA: TIntegerField
      FieldName = 'CATEGORIA'
      Required = True
    end
    object qry_pedidosDESCRICAO1: TStringField
      FieldName = 'DESCRICAO1'
      Size = 50
    end
    object qry_pedidosVALOR_TOTAL: TFMTBCDField
      FieldName = 'VALOR_TOTAL'
      Precision = 15
      Size = 2
    end
  end
  object Dsp_pedidos: TDataSetProvider
    DataSet = qry_pedidos
    Left = 328
    Top = 33
  end
  object cds_pedidos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'Dsp_pedidos'
    Left = 360
    Top = 33
    object cds_pedidosCOD_EMPRESA: TIntegerField
      FieldName = 'COD_EMPRESA'
    end
    object cds_pedidosCODIGO_SEQ: TIntegerField
      FieldName = 'CODIGO_SEQ'
      Required = True
    end
    object cds_pedidosCOD_PRODUTO: TIntegerField
      FieldName = 'COD_PRODUTO'
    end
    object cds_pedidosCOD_USUARIO: TIntegerField
      FieldName = 'COD_USUARIO'
    end
    object cds_pedidosDATA_FALTA: TDateField
      FieldName = 'DATA_FALTA'
    end
    object cds_pedidosFRACAO: TIntegerField
      FieldName = 'FRACAO'
    end
    object cds_pedidosQUANT_FRACAO: TIntegerField
      FieldName = 'QUANT_FRACAO'
    end
    object cds_pedidosQUANT_INTEIRA: TIntegerField
      FieldName = 'QUANT_INTEIRA'
    end
    object cds_pedidosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object cds_pedidosPRECO_CUSTO: TFMTBCDField
      FieldName = 'PRECO_CUSTO'
      currency = True
      Precision = 15
      Size = 2
    end
    object cds_pedidosPRECO_VENDA: TFMTBCDField
      FieldName = 'PRECO_VENDA'
      Precision = 15
      Size = 2
    end
    object cds_pedidosPRECO_VENDA2: TFMTBCDField
      FieldName = 'PRECO_VENDA2'
      Precision = 15
      Size = 2
    end
    object cds_pedidosMED_GENERICO: TStringField
      FieldName = 'MED_GENERICO'
      Size = 3
    end
    object cds_pedidosQTD_FRACIONADA: TFMTBCDField
      FieldName = 'QTD_FRACIONADA'
      Precision = 15
      Size = 2
    end
    object cds_pedidosQTD_CAIXA: TFMTBCDField
      FieldName = 'QTD_CAIXA'
      Precision = 15
      Size = 0
    end
    object cds_pedidosMED_CONTROLADO: TStringField
      FieldName = 'MED_CONTROLADO'
      Size = 1
    end
    object cds_pedidosPRINCIPIO_ATIVO: TStringField
      FieldName = 'PRINCIPIO_ATIVO'
      Size = 50
    end
    object cds_pedidosLABORATORIO: TStringField
      FieldName = 'LABORATORIO'
      Size = 50
    end
    object cds_pedidosCOD_LABORATORIO: TIntegerField
      FieldName = 'COD_LABORATORIO'
    end
    object cds_pedidosCODIGO_MARCA: TIntegerField
      FieldName = 'CODIGO_MARCA'
    end
    object cds_pedidosNOME_GRUPO: TStringField
      FieldName = 'NOME_GRUPO'
      Size = 40
    end
    object cds_pedidosNOME_SUBGRUPO: TStringField
      FieldName = 'NOME_SUBGRUPO'
      Size = 40
    end
    object cds_pedidosNOME_MARCA: TStringField
      FieldName = 'NOME_MARCA'
      Size = 40
    end
    object cds_pedidosCODIGO_SUBGRUPO: TIntegerField
      FieldName = 'CODIGO_SUBGRUPO'
    end
    object cds_pedidosCODIGO_GRUPO: TIntegerField
      FieldName = 'CODIGO_GRUPO'
    end
    object cds_pedidosCATEGORIA: TIntegerField
      FieldName = 'CATEGORIA'
      Required = True
    end
    object cds_pedidosDESCRICAO1: TStringField
      FieldName = 'DESCRICAO1'
      Size = 50
    end
    object cds_pedidosVALOR_TOTAL: TFMTBCDField
      FieldName = 'VALOR_TOTAL'
      currency = True
      Precision = 15
      Size = 2
    end
  end
  object Dts_pedidos: TDataSource
    DataSet = cds_pedidos
    Left = 400
    Top = 33
  end
end
