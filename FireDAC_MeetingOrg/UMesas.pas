...
uses
  ...
  AlignEdit, Mask, DB, IBCustomDataSet, IBQuery, IBSQL,
  IBDatabase, CurvyControls, ImgList, acAlphaImageList, StrUtils, acPNG, ACBrDevice,
  IniFiles, rxToolEdit, rxCurrEdit, UGrupoTeclas, rlutils,
  System.ImageList, frxClass;
...
begin
   if DM.IBTRServer.InTransaction then
...
begin
  ...
  cTipo := '';
   ...
   if ( ssCtrl in Shift ) and (Chr( Key ) in ['J', 'j'] ) and ( iCodigoMesa = 0 ) then
       begin
          if InputQuery('Juntar Mesas', 'Digite o Número da mesa para Juntar', StrMesa) then
              begin
...
var
  ...
  I: Integer;
begin
   if not InputQuery( 'Criar Mesas', 'Digite o número da mesa inicial :', MesaIni ) then
      Exit;
   if not InputQuery( 'Criar Mesas', 'Digite o número da mesa final :', MesaFim ) then
      Exit;
...
begin
   if TRStatus.InTransaction then
      TRStatus.Commit;
...
{   if (iAuxCOO > 0) and (iauxECF > 0) then
 ...
 if Application.MESSAGEBOX('Confirma a Geração da Venda e Fechamento da mesa?', 'Confirmar', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON1) = ID_YES then
begin
...
begin
  inherited;
  FConec := TIBDatabase.Create( nil );
  FConec.DatabaseName := DM.IBDatabase.DatabaseName;
  FConec.Params.Clear;
  FConec.Params.Assign( dm.IBDatabase.Params );
  FConec.LoginPrompt := False;
  FConec.Connected := true;
  FTransaction := TIBTransaction.Create( nil );
  FTransaction.DefaultDatabase := FConec;
