...
uses
  ...
  SUIButton, StdCtrls, Mask, ToolEdit, DB, DBTables, SUIDlg, DBCtrls,
  FMTBcd, SqlExpr, DBClient, SimpleDS, CurrEdit, Mylabel, XDBGrids, sPanel,
  ...
  FireDAC.Comp.Client, Ubibli1;

type
  TFormCRCrediarioConv = class(TForm)
    Panel5: TPanel;
    Panel2: TPanel;
    Panel3: TPanel;
    Panel6: TPanel;
    ...
    RxLabel5: TRxLabel;
    RxLabel6: TRxLabel;
    SPC_BAIXA_PARCELA: TSQLStoredProc;
    SPC_Baixa_Parcial: TSQLStoredProc;
    Panel9: TPanel;
...
var
  FormCRCrediarioConv: TFormCRCrediarioConv;
...
uses ModulodeDadosBaixas, ConsClientes, ModulodeDados, Z_RotinasGerais,
  ...
  Itensdavenda, baixaCR, CrRecebido, FichaClientes, CRCrediario,
  UConsConvenio;

{$R *.dfm}

procedure TFormCRCrediarioConv.FormClose(Sender: TObject;
...
Action:=CaFree;
FormCRCrediarioConv:=nil;

end;

procedure TFormCRCrediarioConv.FormShow(Sender: TObject);
begin
...
procedure TFormCRCrediarioConv.ClienteButtonClick(Sender: TObject);
Var
...
procedure TFormCRCrediarioConv.BTBaixaParcelaClick(Sender: TObject);
begin
  Try
if FormCrRecebido =nil   then
   ...
   Totais.SQL.Text := 'select SUM(VALOR_TOTAL) as total, SUM(VALOR_COMPRA) as COMPRA, SUM(VALOR_JUROS) as JUROS, SUM(VALOR_ACRESCIMO) as MULTA, SUM(VALOR_PAGO) as PAGO,'+
                      'SUM(VALOR_DESCONTO) as DESC from CREDIARIO where crediario.chek_box = ''T'' and COD_CONV =:CLI AND SITUACAO = ''A'' and COD_EMPRESA=:CODEMP';
...
procedure TFormCRCrediarioConv.BTBaixaTotalClick(Sender: TObject);
begin
...
CurrencyEdit9.Value:= 0;
   ...
   ' COD_CONV =:VENDA AND SITUACAO = ''A'' and COD_EMPRESA=:CODEMP';
...
procedure TFormCRCrediarioConv.BTBaixaParcialClick(Sender: TObject);
begin
...
procedure TFormCRCrediarioConv.Edit1Exit(Sender: TObject);
begin
CorSaida(Sender);
end;

procedure TFormCRCrediarioConv.Edit2Exit(Sender: TObject);
begin
CorSaida(Sender);
end;

procedure TFormCRCrediarioConv.Edit3Exit(Sender: TObject);
begin
CorSaida(Sender);
end;

procedure TFormCRCrediarioConv.Edit1Enter(Sender: TObject);
begin
CorEntrada(Sender);
end;

procedure TFormCRCrediarioConv.Edit2Enter(Sender: TObject);
begin
CorEntrada(Sender);
end;

procedure TFormCRCrediarioConv.Edit3Enter(Sender: TObject);
begin
CorEntrada(Sender);
end;

procedure TFormCRCrediarioConv.ClienteEnter(Sender: TObject);
begin
 CorEntrada(Sender);
 RadioButton1.Checked := False;
end;

procedure TFormCRCrediarioConv.BTJurosClick(Sender: TObject);
var
X: Real;
Int: Integer;
begin
...
DMB.SDS_Crediario_Baixa.Next;
 ...
 DMB.Qry_Crediario.CommandText :='select * from V_CREDIARIO_BOLETOS where CODIGO_CLIENTE =:CLI order by data_vencimento asc';
...
procedure TFormCRCrediarioConv.BTPrintClick(Sender: TObject);
var
  Clie: String;
  Cli3: String;
begin
...
procedure TFormCRCrediarioConv.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
if key=#27 then
begin
...
procedure TFormCRCrediarioConv.DBGrid1DblClick(Sender: TObject);
begin
Try
...
procedure TFormCRCrediarioConv.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFormCRCrediarioConv.suiButton1Click(Sender: TObject);
begin
TRY
...
procedure TFormCRCrediarioConv.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TFormCRCrediarioConv.RadioButton1Click(Sender: TObject);
begin
if RadioButton1.Checked then
begin
 ...
 DMB.SDS_Crediario_Baixa.Close;
 DMB.Qry_Crediario.Close;
 DMB.Qry_Crediario.CommandText :='select * from v_crediario_recebido_1 where CODIGO_CLIENTE =:CLI order by data_vencimento asc';
 DMB.Qry_Crediario.Params.Parambyname('CLI').AsInteger    := StrToInt(Cliente.TEXT);
 DMB.Qry_Crediario.Open;
 DMB.SDS_Crediario_Baixa.Open;
...
procedure TFormCRCrediarioConv.DBGrid1CellClick(Column: TXColumn);
begin
...
procedure TFormCRCrediarioConv.suiButton3Click(Sender: TObject);
begin
TRY
...
procedure TFormCRCrediarioConv.RadioButton3Click(Sender: TObject);
begin
{try
...
procedure TFormCRCrediarioConv.RadioButton2Click(Sender: TObject);
begin
{try
DMB.SDS_Crediario_Baixa1.Filtered := FALSE;
dbgrid1.cursor:=crhourglass;
...
procedure TFormCRCrediarioConv.DBGrid1Enter(Sender: TObject);
begin
//FormCRCrediario.KeyPreview := False;
end;

procedure TFormCRCrediarioConv.DBGrid1Exit(Sender: TObject);
begin
//FormCRCrediario.KeyPreview := True;
end;

procedure TFormCRCrediarioConv.ClienteKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFormCRCrediarioConv.suiButton2Click(Sender: TObject);
begin
 Try
...
procedure TFormCRCrediarioConv.ClienteChange(Sender: TObject);
begin
...
procedure TFormCRCrediarioConv.suiButton4Click(Sender: TObject);
Var
Data, DataVenc, DataAtual, diasVenc: TDateTime;
dia, mes, ano: Word;
TOT, Pert, TotPert, dias : Real;
...
BEGIN
    ...
    BEGIN
     DMB.SDS_Crediario_Baixa.Close;
     DMB.Qry_Crediario.Close;
     DMB.Qry_Crediario.CommandText :='select * from CREDIARIO where COD_CONV =:CLI AND SITUACAO = ''A'' order by data_vencimento asc';
     DMB.Qry_Crediario.Params.Parambyname('CLI').AsInteger    := StrToInt(Cliente.TEXT);
     DMB.Qry_Crediario.Open;
     DMB.SDS_Crediario_Baixa.Open;
    ...
    DMB.Sds_Itens_venda.Open;
      ...
      dm.SDS_Clientes.Active := false;
      dm.SDS_Clientes.SQL.Clear;
      dm.SDS_Clientes.SQL.ADD('select * from clientes where COD_CONV = ' + Cliente.Text + ' order by NOME_RS ASC');
      dm.SDS_Clientes.Active := True;

      ACHOU:= DM.SDS_Clientes.locate('COD_CONV', RemoveChar (Cliente.Text),[]);

     //DBComboCliente.Text := FormatFloat('0000',StrToFloat(DBComboCliente.Text));

      ACHOU:= DM.SDS_Clientes.locate('CODIGO', Cliente.Text,[]);

    RXLabel6.Caption := consulta1('convenio', cliente, 'cod_con', 'nome_con', dm.IBTransaction, dm.qConsulta);
    RXLabel5.Caption:= DM.SDS_ClientesCOD_CONV.Text;
...
DMB.SDS_Crediario_Baixa.First;
   ...
   Totais.Close;
   Totais.SQL.Text := 'select SUM(VALOR_TOTAL) as VALOR_TOTAL from CREDIARIO where COD_CONV =:CLI AND SITUACAO = ''A'''; // and COD_EMPRESA=:CODEMP';
   Totais.Parambyname('CLI').AsInteger    := StrToInt(Cliente.TEXT);
//   Totais.ParamByName('CODEMP').AsInteger := dm.SDS_EmpresaCODIGO.AsInteger;
   Totais.Open;
  // Total1 := Totais.FieldByName('SUM').AsFloat;
...
DMB.SDS_Crediario_Baixa.Refresh;
   ...
   with dm.QConsulta do
      begin
         close;
         sql.Clear;
         sql.Text:= 'update crediario set crediario.chek_box = ''F'' WHERE SITUACAO = ''A'' AND COD_CONV =:VENDA';
            Parambyname('VENDA').AsInteger    := StrToInt(Cliente.TEXT);
         ExecSQL;
      end;
   dm.IBTransaction.Commit;
...
procedure TFormCRCrediarioConv.ClienteExit(Sender: TObject);
begin
...
procedure TFormCRCrediarioConv.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
if Key = #13 then
begin
{DMB.SDS_Crediario_Baixa.Edit;
...
procedure TFormCRCrediarioConv.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFormCRCrediarioConv.CurrencyEdit1Change(Sender: TObject);
begin
