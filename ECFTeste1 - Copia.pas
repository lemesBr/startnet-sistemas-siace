...
uses ACBrECF, ACBrBase, ACBrDevice, ACBrECFClass, ACBrConsts,
  ...
  SysUtils, Classes, Graphics, FMTBcd,
  Controls, Forms, Dialogs, StdCtrls,  ComCtrls, Buttons, ExtCtrls,
  Menus, Spin, jpeg, OleCtrls, SHDocVw, DB, IBCustomDataSet, IBQuery, Messages,
    Windows,  Variants,  Mask;
 // {$IFDEF Delphi7}  {$ENDIF};
...
uses ACBrUtil, ACBrECFBematech, VendeItem, EfetuaPagamento,
     ...
     Z_DeclaracoesBemaFi32, ModulodeDados, Principal;
...
begin
 ...
 begin
                    ...
                    qrcupom.parambyname('CARTAODEB').ASFLOAT := 0;
                    qrcupom.parambyname('PROMISSORIA').ASFLOAT := 0;
                    qrcupom.parambyname('CLIENTE').ASSTRING := '1-REDUCACOZ';
                    qrcupom.parambyname('ENDERECO').VALUE := '-';
                    qrcupom.parambyname('CIDADE').ASSTRING := '-';
                    qrcupom.parambyname('CPF').VALUE := '-';
                    qrcupom.parambyname('CONVENIO').VALUE := '-';
...
begin
  ...
  begin
                    ...
                    dm.IBTransaction.StartTransaction;
                    qrcupom.close;
                    qrcupom.parambyname('comando').AsString := 'CANCELARCUPOM';
                    qrcupom.parambyname('NUMERO').VALUE := 0;
                    qrcupom.parambyname('DESCONTO').ASFLOAT := 0;
                    qrcupom.parambyname('ACRESCIMO').ASFLOAT := 0;
                    qrcupom.parambyname('DINHEIRO').ASFLOAT := 0;
                    ...
                    qrcupom.parambyname('CARTAODEB').ASFLOAT := 0;
                    qrcupom.parambyname('PROMISSORIA').ASFLOAT := 0;
                    qrcupom.parambyname('CLIENTE').ASSTRING := '1-CANCELA CUPOM';
                    qrcupom.parambyname('ENDERECO').VALUE := '-';
                    qrcupom.parambyname('CIDADE').ASSTRING := '-';
                    qrcupom.parambyname('CPF').VALUE := '-';
                    qrcupom.parambyname('CONVENIO').VALUE := '-';
  ...
  if InputQuery('Enviar Comando',
    ...
    if (not (ACBrECF1.Modelo = ecfBematech)) or
       InputQuery('Enviar Comando BEMATECH',
                  ...
                  'NAO FDcione os 3 bytes de ACK+ST1+ST2',
                  T ) then
     begin
        CMD2   := '' ;
        A      := 1 ;
