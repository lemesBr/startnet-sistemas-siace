...
type
  Tfrmcontaspagar = class(TForm)
    ...
    qrcontaspagarVALORPAGO: TFMTBCDField;
    qrcontaspagarLIQUIDO: TFMTBCDField;
    qrcontaspagarDESCONTO: TFMTBCDField;
    qrcontaspagarACRESCIMO: TFMTBCDField;
    qrcontaspagarCOD_EMPRESA: TIntegerField;
    qrgradeDESCONTO: TFMTBCDField;
    qrgradeMULTA: TFMTBCDField;
    qrgradeJUROS: TFMTBCDField;
    qrgradeVALOR_DOCUMENTO: TFMTBCDField;
    qrgradeVALOR_PARCELA: TFMTBCDField;
    qrgradeVALOR_BAIXA: TFMTBCDField;
    qrgradeVALOR_ADIANTAMENTO: TFMTBCDField;
    qrgradeVALOR_AUTORIZADO: TFMTBCDField;
    qrgradeVALOR_PAGAR: TFMTBCDField;
...
begin
 ...
 DM.SDS_PlContas.DataSet.CommandText :=('SELECT * FROM PLANOCONTAS WHERE INDICADOR = ''A''');
  ...
  qrcontaspagar.sql.Add
    ('SELECT * FROM CONTAS_APAGAR WHERE COD_EMPRESA =:COD_EMP AND SITUACAO = 1 ORDER BY DATA_VENCIMENTO');
...
begin
  ...
  qrcontaspagar.close;
  qrcontaspagar.sql.clear;
  qrcontaspagar.sql.Add('select * from CONTAS_APAGAR where codigo is not null and Cod_empresa =:cod_emp ' +
...
begin
  ...
  begin
    qrnota.close;
    qrnota.sql.clear;
    qrnota.sql.Add('select * from notasfiscais where CODIGO_NOTA = ' + qrcontaspagarcodnota.Text );
    qrnota.open;
   if qrnota.recordcount > 0 then
    begin
      qritem.close;
      qritem.sql.clear;
      qritem.sql.Add('select * from notasfiscais_itens where CODIGO_NOTA = ' + qrcontaspagarcodnota.Text + ' order by CODIGO_ITEM');
      qritem.open;
...
begin
 ...
 // if frmprincipal.autentica_caixa('Acesso ao Caixa', 0) then
   ...
   begin
      ...
      frmcontaspagar_pgto.DateEdit1.Date :=  Date;
