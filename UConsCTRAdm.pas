...
uses
  ...
  Dialogs, ComCtrls, ExtCtrls, DB, IBCustomDataSet, IBQuery, Grids,
  DBGrids, StdCtrls, AlignEdit, Buttons;

type
  TFrmConsCTRAdm = class(TForm)
    Panel1: TPanel;
    GroupBox1: TGroupBox;
    ComboCons: TComboBox;
    GB3: TGroupBox;
    ...
    QConsRecAdmVALOR: TIBBCDField;
    QConsRecAdmVALOR_PAGO: TIBBCDField;
    QConsRecAdmACRESCIMO: TIBBCDField;
    QConsRecAdmDESCONTO: TIBBCDField;
    QConsRecAdmTAXA: TIBBCDField;
    QConsRecAdmLIQUIDO: TIBBCDField;
    QConsRecAdmADMINISTRADORA: TIBStringField;
    QConsRecAdmBANDEIRA: TIBStringField;
    ComboBandeira: TComboBox;
    QConsRecAdmTIPO_CARTAO: TIBStringField;
    ...
    QConsRecAdmNOME_CLI: TIBStringField;
...
implementation
Uses ModulodeDados, Ubibli1, UConsAdmCartaoCredito, ULancCTRAdm,
  ConsClientes;
var  sSql : string;
   ...
   case ComboCons.ItemIndex of
     3: begin
           ...
           if trim(EdtNome.Text) = '' then
              begin
                 showmessage('Banceira não cadastrada');
                 EdtCod.SetFocus;
              end;
        end;
     4: begin
         ...
         case combocons.ItemIndex of
          ...
          2:begin
               if trim(edtcons.Text) = '' then
                  begin
                     showmessage('Digite o Código do Lançamento');
                     edtcons.SetFocus;
                     exit;
                  end;
          ...
          3:begin
               sql.Add(' WHERE C.CODIGO_BANDEIRA = :BANDEIRA AND ' +
                       ' B.CODIGO_ADMINISTRADORA = :FDM AND      ' +
               ...
               Parambyname('FDm').AsInteger      := strtoint(EdtCod.Text);
         ...
         if tag = 0 then
           begin
              if not QConsRecAdmDATA_PAGAMENTO.IsNull then
                 begin
                    showmessage('Esta parcela está paga, ' + #13 +
