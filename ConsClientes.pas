...
uses
  ...
  SUIButton, Grids, DBGrids, SUIDBCtrls, DB, XDBGrids, Buttons,
  RXDBCtrl, Mask, DBCtrls, sBitBtn, FireDAC.Stan.Intf,
  FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf,
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet, FireDAC.Comp.Client,
  RxToolEdit, AdvGlowButton;
...
Function TFormConsClientes.RemoveLetras(Str:String): String;
Const
  ComLetras = 'ABCDEFGHIJKLMNOPQRSTUVXWYZ';
  SemLetras = '                          ';
Var
  x : Integer;
Begin
  ...
  Begin
    if Pos(Str[x],ComLetras)<>0 Then
    begin
      Str[x] := SemLetras[Pos(Str[x],ComLetras)];
    end;
  end;
 ...
 if cbxpesquisa.ItemIndex = 0 then
   begin
        ...
        //  Edit1.SetFocus;
          ...
          dm.SDS_Clientes.Active := false;
          dm.SDS_Clientes.SQL.Clear;
          dm.SDS_Clientes.SQL.Add('select * from clientes where CODIGO = '  + RemoveLetras(Edit1.Text) + ' order by NOME_RS ASC');
          // dm.SDS_Clientes.Filter:='CODIGO =' +Edit1.Text +'';
          dm.SDS_Clientes.Active := True;
