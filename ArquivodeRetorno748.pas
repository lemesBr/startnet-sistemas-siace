...
uses
  ...
  DB, DBClient, SimpleDS, XDBGrids, Provider, ComCtrls, Grids,
  Buttons, FileCtrl,  Data.DBXFirebird, RxCurrEdit,
   ...
   // cNomedoSacado     := Copy(s,325,30);
    ...
    if banco ='' then
     begin
      ...
      //  ShowMessage('banco :' + banco );

     if banco  = '748' then   // SICREDI
     begin
        ...
        begin
         ...
         begin
           ...
           nValorTotal    := nValorTotal + (StrtoInt(cValorRecebido) / 100);
             ...
             QryBuscaCodigoNumeroBoleto.SQL.Clear;
             QryBuscaCodigoNumeroBoleto.SQL.Add('SELECT * FROM CREDIARIO  WHERE NUMBOLETO =:OLD_NOSSONUMERO') ;
     ...
     // If wLinha = 1 then
         begin
          ...
          // PnCgc.Caption := Copy(s,32,14);
           ...
           PnEmpresa.Caption := Copy(S,47,30);
           PnData.Text    := Copy(S,380,2)+ '/' +Copy(S,382,2)+ '/' + '20'+ Copy(S,384,2);
      ...
      If (wTemp = '1') then
       begin
         ...
         begin
           ...
           cCgcCedente       := Copy(s,4,14);
           PnCgc.Caption := Copy(s,4,14);
           PnConta.Caption   := Copy(S,31,6);

           n              :=0;
           wParte         :='';
           //cNossoNumero   := Copy(s,48,9);
           cNossoNumero   := Copy(s,71,11);
           nNossoNumero   := StrToInt(cNossoNumero);
           cDatadoCredito := Copy(s,296,6);
           wTemp          := Copy(s,117,10);
           cValorRecebido := Copy(s,254,13);
           ...
           nValorTotal    := nValorTotal + (StrtoInt(cValorRecebido) / 100);
             ...
             QryBuscaCodigoNumeroBoleto.Close;
             QryBuscaCodigoNumeroBoleto.SQL.Clear;
             QryBuscaCodigoNumeroBoleto.SQL.Add('SELECT * FROM CREDIARIO  WHERE NUMBOLETO1 =:OLD_NOSSONUMERO') ;
             QryBuscaCodigoNumeroBoleto.ParamByName('OLD_NOSSONUMERO').AsInteger   := nNossoNumero {FormatFloat('000000000',nNossoNumero)};
             QryBuscaCodigoNumeroBoleto.Open;
...
begin
  ...
  while not Eof(NomeArq) do
       begin
         ...
         cDatadoCredito     := Copy(s,296,6);   // ok
      ...
      //   If wTemp = '1' then
       ...
       //  If (Copy(s,319,2) = '00') or (Copy(s,319,2) = 'H5') then
        cNumerodoCedente := IntToStr(dmcob.sds_CedentesCedenteCodigo.AsInteger);

     //   ShowMessage('teste1 cedente := ' + cNumerodoCedente);

           dmcob.sds_cedentes.Locate('CEDENTECODIGO',cNumerodoCedente,[]);

          If (wTemp = '1') then
            If (Copy(s,109,2) = '06')  OR (Copy(s,109,2) = '17') OR (Copy(s,109,2) = '15') OR (Copy(s,109,2) = '16') then
            begin
              wTemp             := Copy(s,1,1);
              cDatadoCredito    := Copy(s,296,6);   // ok
              cDocumento        := Copy(s,117,10);
          ...
          // If wTemp = '1' then
             if (cNumerodoCedente = IntToStr(dmcob.sds_CedentesCedenteCodigo.AsInteger)) and
                ...
                //   wTemp        := Copy(s,117,10);
                   cNossoNumero := Copy(s,72,10);
                   nNossoNumero := StrToInt(cNossoNumero);

                 // ShowMessage('teste3 nosso numero := ' + cNossoNumero);
                 ...
                 //     end; //trim
                  ...
                  // cAnoVcto          := '20' + Copy(s,300,2); // precisa ser Data de Vencimento da Mensalidade
                   ...
                   QryBuscaCodigoNumeroBoleto.SQL.Clear;
                   QryBuscaCodigoNumeroBoleto.SQL.Add('SELECT * FROM CREDIARIO  WHERE NUMBOLETO1 =:OLD_NOSSONUMERO') ;
                   QryBuscaCodigoNumeroBoleto.ParamByName('OLD_NOSSONUMERO').AsInteger   := nNossoNumero;
                   QryBuscaCodigoNumeroBoleto.Open;
                        ...
                        If wTemp <> '' then
                           begin
                            ...
                            // TblRetornoSicredi.FieldByName('DATALIQUIDACAO').asDateTime   := StrToDateTime(wTemp); //  wDATA DE CREDITO DESTA LIQUIDACAO
                             cDataLiquidacao  := Copy(s,296,2) + '/' + Copy(s,298,2)  + '/' +'20'+  Copy(s,300,2);
                            ...
                            // cAnoLiquidacao   := '20 '+ Copy(s,333,2);
                             cAnoLiquidacao   := '20' + Copy(s,300,2);
                             cMesLiquidacao   := Copy(s,298,2);
...
begin
  ...
  QryBuscaCodigoBoleto.SQL.Clear;
  QryBuscaCodigoBoleto.SQL.Add('SELECT * FROM CREDIARIO WHERE NUMBOLETO =:OLD_NOSSONUMERO') ;
  ...
  QryBuscaCodigoBoleto.Close;
  QryBuscaCodigoBoleto.SQL.Clear;
  QryBuscaCodigoBoleto.SQL.Add('SELECT * FROM CREDIARIO  WHERE NUMBOLETO1 =:OLD_NOSSONUMERO') ;
  QryBuscaCodigoBoleto.ParamByName('Old_NossoNumero').AsInteger   := nNossoNumero;
  QryBuscaCodigoBoleto.Open;
  end;
       ...
       IBBoleto.Close;
        ...
        dm.ProxCod.Close;
                   ...
                   with DM.QConsulta do
                    ...
                    parambyname('COD_CREDIAIRIO').AsInteger   := QryBuscaCodigoBoletoCodigo.value;
                    parambyname('COD_EMPRESA').AsInteger      := dm.SDS_EmpresaCODIGO.value;
                    parambyname('PARCELA').asString           := QryBuscaCodigoBoletoPARCELA.Text;
                    parambyname('VALOR_PARCELA').AsFloat      := QryBuscaCodigoBoletoVALOR_COMPRA.AsFloat;
                    parambyname('JUROS').asfloat              := cJuros;
                    parambyname('MULTA').AsFloat              := cMulta;
                    parambyname('DESCONTO').AsFloat           := QryBuscaCodigoBoletoVALOR_DESCONTO.AsFloat;
                    parambyname('VALOR_TOTAL').AsFloat        := wValorRecebido;
                    parambyname('DATA_BAIXA').AsDate          := StrToDateTime(cDataLiquidacao);
                    parambyname('USUARIO').AsString           := FormPrincipal.UserLogado;
                    parambyname('COD_CLIENTE').AsInteger      := QryBuscaCodigoBoletoCODIGO_CLIENTE.asinteger;
                    parambyname('NOME_CLIENTE').AsString      := QryBuscaCodigoBoletoNOME_SACADO.Text;
                    parambyname('COD_COMPRA').AsString        := QryBuscaCodigoBoletoCODIGO_COMPRA.Text;
                    parambyname('SALDO_PAGAR').AsFloat        := 0;
                    parambyname('DATA_VENCIMENTO').asdate     := QryBuscaCodigoBoletoData_Vencimento.AsDateTime;
                    parambyname('VALOR_PAGO').AsFloat         := QryBuscaCodigoBoletoVALOR_TOTAL.AsFloat;
...
begin
  ...
  FileListBox1.ApplyFilePath(wPathTemp + '\*.CRT;*.RET');
  ...
  begin
    ...
    //  wPathTemp           :=   Trim(cCaminho) + Trim(FrmMenuPrincipal.cRetorno);
      ...
      If Trim(wPathTemp) = '' then
         wPathTemp := 'C:\StartNet_Sistemas\Retorno\';
...
begin
  Porta_Saida := 'C:\StartNet_Sistemas' + wArqAtu + 'ERR.TXT';
...
begin
 // PnConta.Caption := FormatFloat('000',FrmMvtoBancario.IBContasBancarias.FieldbyName('NumerodaConta').asInteger);
  ...
  FileListBox1.ApplyFilePath(Trim(FormPrincipal.cCaminho.Text) + '*.CRT;*.RET' );

  PnPath.Caption := Trim(FormPrincipal.cCaminho.Text) + '*.CRT*.RET';
