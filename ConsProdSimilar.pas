...
uses
  ...
  RXCtrls, DBCtrls, SUIDBCtrls,  Mask, ACBrBase, ACBrLCB, Buttons,
  sBitBtn, sPanel, sDBNavigator, AdvGlowButton, sLabel;
...
xDBGrid1.SetFocus;
        ...
        IF Edit2.Text = 'LETRAS' then
         ...
         DM.Sds_produtos.Active := false;
         DM.Sds_produtos.sql.clear;
         DM.Sds_produtos.sql.add('select * from PRODUTOS where CODIGO like ' + #39 + '%' + RemoveChar(Edit1.Text) + '%' + #39+'ORDER BY DESCRICAO');
         DM.Sds_produtos.Active := True;
         Label1.Caption := 'N° Produtos Ativos Fitrados: ' + IntToStr(DM.Sds_produtos.RecordCount);

            If DM.Sds_produtos.locate('CODIGO', RemoveChar(Edit1.Text),[])=True then
            begin
            XDBGRID1.SetFocus;
            end else
            If not DM.Sds_produtos.locate('CODIGO', RemoveChar(Edit1.Text),[])=True then
            begin
             SHOWMESSAGE('Produto Não Localizado');
             Edit1.SetFocus;
            end;
