...
begin
  ...
  if not DirectoryExists(ExtractFilePath(fMenu.coneccao.DatabaseName)+'\'+'BKP') then
    ForceDirectories(Pchar(ExtractFilePath(fMenu.coneccao.DatabaseName)+'\'+'BKP'));

  SaveDialog1.InitialDir := Pchar(ExtractFilePath(fMenu.coneccao.DatabaseName)+'\'+'BKP');
  ...
  begin
    Attach;
    DatabaseName := fMenu.coneccao.DatabaseName;
    BackupFile.Clear;
...
begin
  ...
  begin
    ...
    fMenu.coneccao.Connected := False;
    ...
    begin
      Attach;
      DatabaseName.Clear;
      DatabaseName.Add( fMenu.coneccao.DatabaseName );
