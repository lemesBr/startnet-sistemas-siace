...
begin
   ...
   with dm.QCaixa  do
      begin
         close;
         sql.Clear;
         sql.Text:= 'SELECT * FROM USUARIOS WHERE USUARIO =:USER ORDER BY CODIGO';
         ParamByName('USER').AsString := FormPrincipal.UserLogado;
         open;
         First;
         ...
         while not eof do
            begin
               combocaixa.Items.Add(dm.QCaixaCODIGO.AsString + '-' + dm.QCaixaUSUARIO.Value);
               next;
            end;
      end;
   dm.IBTransaction.Commit;
