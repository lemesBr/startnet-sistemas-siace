...
begin
 ...
 with formVendas.QryBicoEnc_Temp do
  begin
   Close;
   UnPrepare;
...
begin
 if formVendas.QryBicoEnc_Temp.RecordCount <> 0 then
  if formVendas.QryBicoEnc_Temp['ID_BICO'] <> '0' then
   Begin
    {if formVendas.QryBicoEnc_Temp['COD_FRENTISTA'] <> '0' then
    VendaFrm.DBLookupComboBox1.KeyValue := formVendas.QryBicoEnc_Temp['COD_FRENTISTA'];}

         bomba  := formVendas.QryBicoEnc_Temp['ID_bomba'];
         tamque := formVendas.QryBicoEnc_Temp['ID_tanque'];

         dm.SDS_PRODUTOS_c.Active := false;
         DM.Sds_produtos_c.DataSet.CommandText:= 'select * from PRODUTOS where codigo like ' + #39 + '%' + formVendas.QryBicoEnc_Temp.FieldbyName('ID_PRODUTOS').AsString + '%' + #39+'ORDER BY DESCRICAO';
         dm.SDS_PRODUTOS_c.Active := True;

          If DM.SDS_PRODUTOS_C.locate('codigo', formVendas.QryBicoEnc_Temp.FieldbyName('ID_PRODUTOS').AsString,[])=True then
          begin
         ...
         // formvendas.sds_pedidos_itens.insert;
          ...
          formvendas.Sds_pedidos_itensPreco_unitario.text     := formatcurr('0.000', formVendas.QryBicoEnc_Temp['PRECO']);
          formvendas.sds_pedidos_itensDescricao_PRODUTO.text  := formVendas.QryBicoEnc_Temp['PRODUTOS_DESC'];
          formvendas.Sds_pedidos_itensquantidade.text         := formatcurr('0.000', formVendas.QryBicoEnc_Temp['LITROS']);
          formvendas.Sds_pedidos_itensDESCONTO.Text           := '0,00';
          formvendas.Sds_pedidos_itensCODIGO.AsInteger        := FORMVENDAS.NItem;
          formvendas.Sds_pedidos_itensCODIGO_PROD.Text        := formVendas.QryBicoEnc_Temp.FieldbyName('ID_PRODUTOS').AsString;
          ...
          formvendas.Sds_pedidos_itensCODIGO_BICO.Text        := formVendas.QryBicoEnc_TempCOD_BICOENC_TEMP.Text;
          ...
          formvendas.sds_pedidos_itensBOMBA.Text              := formVendas.QryBicoEnc_TempID_BOMBA.Text;
          formvendas.sds_pedidos_itensTANQUE.Text             := formVendas.QryBicoEnc_TempID_TANQUE.Text;
          formVendas.sds_pedidos_itensBICO.text               := formVendas.QryBicoEnc_TempID_BICO.Text;
        ...
        //  IF not formvendas.sds_pedidos_itens.Eof then
         // begin
          ...
          formvendas.ComboEdit1.SetFocus;
              ...
              If not DM.SDS_PRODUTOS_C.locate('codigo', formVendas.QryBicoEnc_Temp.FieldbyName('ID_PRODUTOS').AsString,[])=True then
              begin
   ...
   { VendaFrm.Listview1.Items.Insert(0);
    VendaFrm.Listview1.Items.Item[0].Caption := formVendas.QryBicoEnc_Temp.FieldbyName('ID_PRODUTOS').AsString ;
    VendaFrm.Listview1.Items.Item[0].SubItems.Insert(0, formatcurr('0.000', formVendas.QryBicoEnc_Temp['LITROS']));
    VendaFrm.Listview1.Items.Item[0].SubItems.Insert(1, formVendas.QryBicoEnc_Temp['PRODUTOS_DESC']);
    VendaFrm.Listview1.Items.Item[0].SubItems.Insert(2, formatcurr('0.000', formVendas.QryBicoEnc_Temp['PRECO']));
    VendaFrm.Listview1.Items.Item[0].SubItems.Insert(3, formatcurr('0.00', formVendas.QryBicoEnc_Temp['DINHEIRO']));
    ...
    VendaFrm.Listview1.Items.Item[0].SubItems.Insert(8, formVendas.QryBicoEnc_Temp.FieldbyName('ID_PRODUTOS').AsString); //Cod_produtos
    ...
    VendaFrm.Listview1.Items.Item[0].SubItems.Insert(25, formVendas.QryBicoEnc_Temp['COD_BICOENC_TEMP']);
...
begin
// DM.CloseIBArray([formVendas.QryBicoEnc_Temp, VendaBicoEnc_TempListFrm.QryBomba]);
 action := cafree;
end;
...
begin
 ...
 if formVendas.QryBicoEnc_Temp.Locate('ID_BOMBA', Edit2.Text, []) then
 begin
...
var x : integer;
begin
 formVendas.QryBicoEnc_Temp.Close;
 formVendas.QryBicoEnc_Temp.SQL.Clear;
 formVendas.QryBicoEnc_Temp.SQL.Add('select * from BICOENC_TEMP where COD_BICOENC_TEMP > 0');
 for x := 0 to FORMVENDAS.Sds_pedidos_itens.recordcount - 1 do
 begin
  if formvendas.sds_pedidos_itensCodigo_bico.text <> '' then
  begin
    formVendas.QryBicoEnc_Temp.SQL.Add('and COD_BICOENC_TEMP <> :COD_BICOENC_TEMP' + inttostr(x));
    formVendas.QryBicoEnc_Temp.ParamByName('COD_BICOENC_TEMP' + inttostr(x)).AsString :=
               formvendas.sds_pedidos_itensCodigo_bico.text;
  end;
 end;
formVendas.QryBicoEnc_Temp.Prepare;
formVendas.QryBicoEnc_Temp.Open;
...
{ while not QryBomba.Eof do
 ...
 formVendas.formVendas.QryBicoEnc_Temp.First;

 while not formVendas.formVendas.QryBicoEnc_Temp.Eof do
  begin
    TImage(VendaBicoEnc_TempListFrm.FindComponent('Estado' + inttostr(formVendas.formVendas.QryBicoEnc_Temp.FieldByName('ID_BOMBA').AsInteger - 1))).Picture.Bitmap := FimDeVendaImg.Picture.Bitmap;
    formVendas.formVendas.QryBicoEnc_Temp.Next;
  end;    }
end;
