...
uses
  ...
  Grids, XDBGrids, DBClient, SimpleDS, sLabel;
...
uses ModulodeDados, Z_CriptDecript, Z_RotinasGerais,
  ...
  PedidoParcela, Vendas, RegPedido;
...
USER:=(edit11.Text);
 ...
 if not (DM.SDS_Usuarios.Locate('Usuario;senha',VarArrayOf([user,SenhaAtual]),[locaseInsensitive]) = True) then
 begin
 ...
 if DM.SDS_Usuarios.locate('Usuario;Senha',VarArrayOf([USER,SenhaAtual]),[locaseInsensitive]) = true then
 begin
 ...
 begin
       ...
       if (DMC.sds_libera.State in [dsInsert, dsEdit]) then
       begin
        DMC.sds_libera.Post;
    ...
    DMC.sds_libera.Delete;
     ...
     if (tag = 0) then
       ...
       begin
        ...
        DMC.sds_liberaVALOR_SOLICITADO.Text := FormCupomFiscal.ClientDataSet1Soma.TEXT;
        DMC.sds_liberaSolicita.text := 'S';
        DMC.sds_liberaAUTO_LIBERA.Text := 'N';
        if (DMC.sds_libera.State in [dsInsert, dsEdit]) then
        begin
...
begin
    ...
    {  if Tag = 1 then
      ...
      DMC.sds_libera.Refresh;
      DMC.sds_libera.Filtered := False;
      DMC.sds_libera.Filter := 'COD_VENDA =' + FormCupomFiscal.Edit2.Caption;
      DMC.sds_libera.Filtered := TRUE;
      end;   }
